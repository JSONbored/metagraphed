 ...
 */
export const YIELD_PERCENTILE_STRIP_GRID_CLASS =
 ...
 */
export function shouldUseYieldPercentileFourColumnLayout(
  widthPx: number,
): boolean {
  if (!Number.isFinite(widthPx) || widthPx <= 0) return false;
  const rootFontPx = 16;
  const thresholdPx =
    parseFloat(YIELD_PERCENTILE_FOUR_COL_MIN_WIDTH) * rootFontPx;
