...
describe("SN57 Sparket call_subnet_surface verification (#7070)", () => {
  ...
  test("callSubnetSurface returns the real JSON body using the surface's own url + GET", async () => {
    ...
    assert.equal(typeof (result.body as Row).status, "string");
    assert.equal(typeof (result.body as Row).last_sync_at, "string");
    assert.equal(typeof (result.body as Row).sync_lag_seconds, "number");
    assert.equal((result.body as Row).db_connected, true);
    assert.equal((result.body as Row).version, "0.1.0");
  });
