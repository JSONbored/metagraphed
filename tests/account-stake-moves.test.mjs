...
// One GROUP BY netuid row (movements count + first/last observed epoch ms).
...
const ADDR = "5GReferenceAccountAddressForStakeMovesTestsssssssss";
...
describe("buildAccountStakeMoves", () => {
  ...
  test("folds per-subnet movement counts + first/last timestamps", () => {
    ...
    // subnet 1 has the most movements (3), so it leads + is dominant.
  ...
  test("skips malformed/blank/negative netuid and zero-count rows", () => {
    const d = buildAccountStakeMoves(
      [
        ...
        row(2, 0, 1000, 2000), // zero movements: skipped
      ],
      ADDR,
      { window: "7d" },
    );
