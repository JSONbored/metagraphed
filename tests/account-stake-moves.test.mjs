...
import {
  ...
  STAKE_MOVE_EVENT_KIND,
  DEFAULT_STAKE_MOVE_WINDOW,
} from "../src/account-stake-moves.mjs";

// One GROUP BY netuid row (move count + first/last observed epoch ms).
...
const ADDR = "5GReferenceAccountAddressForStakeMovesTestsssssss";
...
describe("buildAccountStakeMoves", () => {
  ...
  test("folds per-origin-subnet move counts + first/last timestamps", () => {
    ...
    // subnet 1 has the most moves (3), so it leads + is dominant.
  ...
  test("HHI concentration: all moves off one subnet -> 1, spread -> < 1", () => {
  ...
  test("ties on move count break by netuid ascending", () => {
    const d = buildAccountStakeMoves(
  ...
  test("merges duplicate netuid rows and keeps the widest first/last span", () => {
    ...
    const s = d.subnets[0];
    assert.equal(s.movements, 3); // 2 + 1
    assert.equal(s.first_moved_at, new Date(1000).toISOString()); // min
    assert.equal(s.last_moved_at, new Date(5000).toISOString()); // max
  });
  ...
  test("skips malformed/blank/negative netuid and zero-count rows", () => {
    const d = buildAccountStakeMoves(
      [
        ...
        row(2, 0, 1000, 2000), // zero moves: skipped
      ],
      ADDR,
      { window: "7d" },
    );
...
describe("loadAccountStakeMoves", () => {
  test("seeks the hotkey index for StakeMoved over the window and shapes it", async () => {
    let captured;
    ...
    assert.equal(captured.params[1], STAKE_MOVE_EVENT_KIND);
