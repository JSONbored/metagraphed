...
//
// The surface is the public no-auth SN54 API health endpoint
// (sn-54-yanez-miners-stats-health, GET https://miners-stats.yanezcompliance.net/health, JSON, single fixed endpoint -- no schema). Live-verified
...
// A faithful subset of the live https://miners-stats.yanezcompliance.net/health response body.
...
describe("SN54 SN54 call_subnet_surface verification (#7067)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(
      SURFACE.url,
      "https://miners-stats.yanezcompliance.net/health",
    );
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 54 }],
    };
    const deps = {
