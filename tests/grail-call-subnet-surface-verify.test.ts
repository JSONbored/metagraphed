...
// Grail's official Grafana instance (sn-81-grail-grafana-openapi, GET
// https://grail-grafana.tplr.ai/public/openapi3.json). Live-verified
// 2026-07-21 to return HTTP 200 application/json -- an OpenAPI 3.0.3 document
// (info.title "Grafana HTTP API.", version "0.0.1", 207 documented paths). The
// fixture below mirrors that live response's top-level shape rather than
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/grail.json", import.meta.url)),
    "utf8",
  ),
);
...
describe("SN81 Grail call_subnet_surface verification (#7094)", () => {
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 81 }],
    };
    const deps = {
