...
test("handleSubscribe: a valid uri subscribes, persists, and notifies ChainFirehoseHub", async () => {
  ...
  const hub = new McpSessionHub(
    stubState(),
    mockEnv({ CHAIN_FIREHOSE_HUB: firehose }),
  );
...
test("handleStream: rejects an arbitrary session id that has not subscribed", async () => {
  ...
  assert.equal((state.storage as Row).raw.size, 0);
  assert.equal((state.storage as Row).lastAlarmTime, null);
});
...
test("alarm: self-terminates using the persisted sessionId (no caller to hand it one)", async () => {
  ...
  const revivedHub = new McpSessionHub(
    state,
    mockEnv({ CHAIN_FIREHOSE_HUB: firehose }),
  );
...
test("touch: sets a Durable Object alarm MCP_SESSION_IDLE_TTL_MS in the future", async () => {
  ...
  await hub.touch();
  const alarmTime = (state.storage as Row).lastAlarmTime;
...
test("handleUnsubscribe: subnet status uri unregisters that netuid from SubnetStatusHub", async () => {
  ...
  const hub = new McpSessionHub(
    stubState(),
    mockEnv({ SUBNET_STATUS_HUB: statusHub }),
  );
  await hub.fetch(
    jsonRequest("https://mcp-session-hub.internal/subscribe", {
      sessionId: "session-1",
