...
function makeCtx(): ProfilesCtx {
  return { env: {} } as unknown as ProfilesCtx;
}

function makeDeps({ listBlob = PROFILES_BLOB as Row | null } = {}) {
  return {
    readOptionalArtifact: async (_ctx: unknown, path: string) =>
      path === "/metagraph/profiles.json" ? listBlob : null,
    readArtifact: async (_ctx: unknown, path: string) => {
      if (path === "/metagraph/profiles/7.json") {
        return { subnet: { netuid: 7, slug: "allways" }, profile: PROFILE_ROW };
      }
      const err = profilesMcpError("not_found", "Profile not found.") as Row;
      err.code = "not_found";
      throw err;
    },
  };
...
describe("profiles-mcp — loadSubnetProfile", () => {
  test("loads the per-netuid profile artifact", async () => {
    const out = (await loadSubnetProfile(makeCtx(), 7, makeDeps())) as Row;
...
describe("profiles-mcp — MCP metadata", () => {
  ...
  test("MCP server exports wire profile tools", () => {
    ...
    for (const name of ["list_profiles", "get_subnet_profile"]) {
      const tool = MCP_TOOLS.find((t: Row) => t.name === name);
