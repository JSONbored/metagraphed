...
//
// The surface is the public no-auth SN5 API health endpoint
// (sn-5-hone-subnet-api, GET https://api.hone.training/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/hone.json", import.meta.url)),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.hone.training/health response body.
...
describe("SN5 Hone call_subnet_surface verification (#7021)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.hone.training/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 5 }],
    };
    const deps = {
