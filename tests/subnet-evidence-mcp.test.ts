...
type LoadCtx = Parameters<typeof loadSubnetEvidenceList>[0];
type LoadDeps = Parameters<typeof loadSubnetEvidenceList>[2];
...
describe("subnet-evidence-mcp", () => {
  ...
  test("MCP server exports wire list_subnet_evidence at the bumped SemVer", () => {
    ...
    const tool = MCP_TOOLS.find((t: Row) => t.name === "list_subnet_evidence");
    assert.ok(tool);
