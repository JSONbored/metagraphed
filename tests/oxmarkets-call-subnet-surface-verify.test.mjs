...
//
// The surface is the public no-auth SN35 API health endpoint
// (sn-35-oxmarkets-health, GET https://api.cartha.finance/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/oxmarkets.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.cartha.finance/health response body.
...
describe("SN35 OxMarkets call_subnet_surface verification (#7050)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.cartha.finance/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 35 }],
    };
    const deps = {
