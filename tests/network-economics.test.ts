...
type EconCtx = Parameters<typeof loadNetworkEconomics>[0];
type EconDeps = Parameters<typeof loadNetworkEconomics>[2];
...
function makeCtx({ kv = null as Row | null } = {}): EconCtx {
  return {
  ...
  } as unknown as EconCtx;
}

function makeDeps(artifactData: unknown): EconDeps {
  return {
    contractVersion: () => "test-contract",
    readOptionalArtifact: async () => artifactData,
  } as unknown as EconDeps;
}
...
describe("network-economics — loadNetworkEconomics", () => {
  ...
  test("defaults source when the live tier returns data without a source label", async () => {
    const spy = vi
      ...
      .mockResolvedValue({
        ...
        ReturnType<typeof healthServing.resolveLiveEconomics>
      >);
    try {
  ...
  test("validates list-query args before live-KV or R2 reads", async () => {
    ...
    await assert.rejects(
      () =>
        loadNetworkEconomics(ctx, { netuid: -1 }, deps as unknown as EconDeps),
...
describe("network-economics — MCP surface exports", () => {
  test("declares a compilable tool + output schema", () => {
    ...
    assert.ok(
      (GET_ECONOMICS_MCP_TOOL.inputSchema.properties as Row).sort.enum.length >
        0,
    );
