...
describe("SN28 gm call_subnet_surface verification (#7044)", () => {
  ...
  test("callSubnetSurface returns the real JSON body using the surface's own url + GET", async () => {
    ...
    assert.equal((result.body as Row).object, "list");
    assert.ok(Array.isArray((result.body as Row).data));
    assert.equal(typeof (result.body as Row).data[0].id, "string");
    assert.equal((result.body as Row).data[0].object, "model");
  });
