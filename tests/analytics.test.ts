  ...
  test("economic boards break metric ties by tiebreak then netuid, nulls last", () => {
    ...
    assert.deepEqual(
      ranked("highest-emission", [
        { netuid: 30, emission_share: 0.2, total_stake_alpha: 100 },
        { netuid: 31, emission_share: 0.2, total_stake_alpha: null },
        { netuid: 32, emission_share: 0.2, total_stake_alpha: 999 },
      ]),
      [32, 30, 31],
    );
...
describe("writeSubnetSnapshot", () => {
  ...
  test("still counts structural rows when optional economics read throws", async () => {
    ...
    assert.equal(captured.snapshot[0].total_stake_alpha, null);
