...
const reqWith = (ifNoneMatch: string | null) =>
...
test("dataResponse emits the SuccessEnvelope shape with no error key", async () => {
  const response = dataResponse(mockEnv(), { hello: "world" }, 200, {
    source: "test",
  });
...
test("ifNoneMatchSatisfied: the * form matches any current representation", () => {
  ...
  assert.equal(
    ifNoneMatchSatisfied(reqWith("*"), null as unknown as string),
    false,
  );
});
...
test("ifNoneMatchSatisfied: no header or no etag is a miss", () => {
  ...
  assert.equal(
    ifNoneMatchSatisfied(reqWith(ETAG), null as unknown as string),
    false,
  );
});
