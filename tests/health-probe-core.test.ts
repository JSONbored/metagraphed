...
function rpcBody(id: unknown, result: unknown): string {
...
describe("classifyProbe", () => {
  ...
  test("maps status codes + content to classifications", () => {
    assert.equal(
      classifyProbe(
        { error_class: "AbortError" } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "timeout",
    );
    assert.equal(
      classifyProbe(
        { status_code: 429 } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "rate-limited",
    );
    assert.equal(
      classifyProbe(
        { status_code: 403 } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "auth-required",
    );
    assert.equal(
      classifyProbe(
        { status_code: 404 } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "dead",
    );
    assert.equal(
      classifyProbe(
        { status_code: 503 } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "transient",
    );
    assert.equal(
      classifyProbe(
        { ok: true, content_type: "text/html" } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "live",
    );
    ...
    assert.equal(
      classifyProbe(
        { unsafe_url: true } as unknown as HttpProbeResult,
        htmlSurface,
      ),
      "unsafe",
    );
...
describe("classifyRpcProbe + statusForClassification", () => {
  test("live requires header + system_health", () => {
    ...
    assert.equal(
      classifyRpcProbe({
        method_results: { chain_getHeader: { ok: true } },
      } as unknown as RpcProbeResult),
      "unsupported",
    );
    assert.equal(
...
describe("probeUrl", () => {
  const opts = (
    ...
    isUnsafeUrl: (url: unknown) => boolean | Promise<boolean> = async () =>
      false,
  ) => ({
    fetchImpl,
    isUnsafeUrl,
  ...
  test("redirect to unsafe target is blocked (private_redirect_blocked)", async () => {
    const probe = await probeUrl("https://x.dev/old", "GET", "*/*", 5000, {
      ...
      isUnsafeUrl: async (u) => (u as string).includes("169.254"),
      fetchImpl: async () =>
...
describe("probeSubtensorHttp / jsonRpcHttp (HTTP RPC)", () => {
  ...
  test("transport error: fail-fast after first failed RPC call", async () => {
    ...
    assert.equal((probe as Row).transport_error, true);
  ...
  test("redirect to unsafe target during RPC is a transport error", async () => {
    const probe = await probeSubtensorHttp("https://rpc.dev", 5000, {
      isUnsafeUrl: async (u) => (u as string).includes("127.0.0.1"),
      fetchImpl: async () =>
        fakeResponse({
          status: 308,
    ...
    assert.equal((probe as Row).redirect_target, "http://127.0.0.1:9944/");
...
describe("nodeWebSocketConnector", () => {
  ...
  test("missing WebSocket global rejects", async () => {
    ...
    const connect = nodeWebSocketConnector(null as unknown as typeof WebSocket);
  ...
  test("timeout swallows a close() that throws", async () => {
    ...
    const connect = nodeWebSocketConnector(
      ThrowingCloseSocket as unknown as typeof WebSocket,
    );
    const promise = connect("wss://rpc.dev", calls, 0);
...
describe("parseBlockNumber", () => {
  ...
  test("non-object header returns null", () => {
    ...
    assert.equal(
      parseBlockNumber("nope" as unknown as { number?: unknown }),
      null,
    );
  });
...
describe("contentMismatch (remaining branches)", () => {
  test("json expect: text/plain at a .json path is tolerated", () => {
    assert.equal(
      contentMismatch(
        { content_type: "text/plain" } as unknown as HttpProbeResult,
        {
          url: "https://x.dev/file.JSON",
          probe: { expect: "json" },
        } as unknown as ProbeSurface,
      ),
      false,
  ...
  test("json expect: non-json content is a mismatch", () => {
    assert.equal(
      contentMismatch(
        { content_type: "text/html" } as unknown as HttpProbeResult,
        {
          url: "https://x.dev/data.json",
          probe: { expect: "json" },
        } as unknown as ProbeSurface,
  ...
  test("html expect: non-html is a mismatch, html is fine", () => {
    ...
    assert.equal(
      contentMismatch(
        { content_type: "application/json" } as unknown as HttpProbeResult,
        surface,
      ),
      true,
    );
  ...
  test("sse expect: non-sse is a mismatch, event-stream is fine", () => {
    ...
    assert.equal(
      contentMismatch(
        { content_type: "text/event-stream" } as unknown as HttpProbeResult,
        surface,
      ),
      false,
    );
...
describe("classifyProbe / classifyRpcProbe (remaining branches)", () => {
  test("classifyProbe: private_redirect_blocked -> unsafe", () => {
    assert.equal(
      classifyProbe(
        { private_redirect_blocked: true } as unknown as HttpProbeResult,
        { probe: {} } as unknown as ProbeSurface,
      ),
      "unsafe",
    );
  });
  test("classifyProbe: 410 -> dead, 401 -> auth-required", () => {
    const s = { probe: { expect: "html" } } as unknown as ProbeSurface;
    assert.equal(
      classifyProbe({ status_code: 410 } as unknown as HttpProbeResult, s),
      "dead",
    );
    assert.equal(
      classifyProbe({ status_code: 401 } as unknown as HttpProbeResult, s),
      "auth-required",
    );
  });
  test("classifyProbe: not-ok, no special status -> unsupported", () => {
    assert.equal(
      classifyProbe(
        { ok: false, status_code: 200 } as unknown as HttpProbeResult,
        { probe: { expect: "html" } } as unknown as ProbeSurface,
      ),
      "unsupported",
    );
  });
  test("classifyRpcProbe: unsafe / rate-limited / auth-required / transient", () => {
    assert.equal(
      classifyRpcProbe({ unsafe_url: true } as unknown as RpcProbeResult),
      "unsafe",
    );
    assert.equal(
      classifyRpcProbe({
    ...
    assert.equal(
      classifyRpcProbe({ status_code: 429 } as unknown as RpcProbeResult),
      "rate-limited",
    );
    assert.equal(
      classifyRpcProbe({ status_code: 401 } as unknown as RpcProbeResult),
      "auth-required",
    );
    assert.equal(
      classifyRpcProbe({ status_code: 500 } as unknown as RpcProbeResult),
      "transient",
    );
    assert.equal(
      classifyRpcProbe({ error: "boom" } as unknown as RpcProbeResult),
      "unsupported",
    );
    assert.equal(
      classifyRpcProbe({ method_results: {} } as unknown as RpcProbeResult),
      "transient",
    );
  });
  ...
  test("classifyRpcProbe: wrong-chain only on explicit genesis mismatch", () => {
    ...
    assert.equal(
      statusForClassification("wrong-chain", {
        authority: "community",
      } as unknown as ProbeSurface),
      "failed",
    );
...
describe("statusForClassification (remaining branches)", () => {
  ...
  test("unsupported with official authority stays failed", () => {
    assert.equal(
      statusForClassification("unsupported", {
        authority: "official",
      } as unknown as ProbeSurface),
      "failed",
    );
...
describe("contentMismatch with null content_type", () => {
  test("json expect + null content_type is a mismatch (|| '' branch)", () => {
    assert.equal(
      contentMismatch(
        { content_type: null } as unknown as HttpProbeResult,
        {
          url: "https://x.dev/data.json",
          probe: { expect: "json" },
        } as unknown as ProbeSurface,
