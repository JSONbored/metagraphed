...
//
// The surface is the public no-auth content-detection API health endpoint
// (sn-32-itsai-subnet-api, GET https://api.its-ai.org/api/health, JSON,
// single fixed endpoint -- no schema). Live-verified 2026-07-21 to return
// HTTP 200 application/json {"success": true}. The fixture below mirrors
// that live response rather than fetching it, keeping the test hermetic
// while still exercising the JSON parse-and-return path.
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/itsai.json", import.meta.url)),
    "utf8",
  ),
);
...
describe("SN32 ItsAI call_subnet_surface verification (#7047)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.its-ai.org/api/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 32 }],
    };
    const deps = {
