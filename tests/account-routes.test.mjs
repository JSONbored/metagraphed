...
test("GET /accounts/{ss58}/stake-moves routes to the per-account stake-moves handler", async () => {
  const env = dbWith({
    events: [
      {
        netuid: 1,
  ...
  const res = await handleRequest(
    req(`/api/v1/accounts/${SS58}/stake-moves?window=30d`),
    env,
    {},
  );
  assert.equal(res.status, 200);
  ...
  assert.equal(body.data.address, SS58);
  assert.equal(body.data.window, "30d");
  assert.equal(body.data.total_movements, 5);
  assert.equal(body.data.subnets[0].netuid, 1); // most moves -> leads + dominant
  assert.equal(body.data.dominant_netuid, 1);
  assert.equal(body.meta.source, "chain-events");
});

test("GET /accounts/{ss58}/stake-moves is schema-stable when D1 is cold (never 404)", async () => {
  const res = await handleRequest(
...
test("GET /accounts/{ss58}/stake-moves rejects an unknown query param with 400", async () => {
  const res = await handleRequest(
    req(`/api/v1/accounts/${SS58}/stake-moves?bogus=1`),
    dbWith({}),
    {},
  );
  assert.equal(res.status, 400);
...
test("GET /accounts/{ss58}/stake-moves rejects an unsupported window with 400", async () => {
  const res = await handleRequest(
    req(`/api/v1/accounts/${SS58}/stake-moves?window=1y`),
    dbWith({}),
    {},
  );
  assert.equal(res.status, 400);
