...
//
// The surface is the public no-auth TaoMarketCap SN27 snapshot API
// (GET https://api.taomarketcap.com/public/v1/subnets/27/, JSON, no schema --
// a single fixed endpoint that rejects HEAD with 405, so it probes with GET).
// Live-verified 2026-07-21 to return HTTP 200 application/json { id, netuid: 27,
// created_at_block, registered_at, is_active, latest_snapshot: { id, netuid,
// ... }, ... }. The fixture below mirrors that live response's shape rather than
// fetching it, keeping the test hermetic. (The snapshot is live chain data, so
// the test asserts the stable shape, not its exact values.)
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/ni-compute.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.taomarketcap.com/public/v1/subnets/27/
// response body.
const SN27_BODY = {
  id: "27",
  netuid: 27,
  created_at_block: 3049740,
  registered_at: "2023-10-04T15:34:00+00:00",
  is_active: true,
  is_subsidized: false,
  mechanism_count: 1,
  latest_snapshot: {
    ...
    subnet_owner_hotkey: "5F4tQyWrhfGVcNhoqeiNsR6KjD4wMZ2kfhLj4oHYuyHbZAc3",
  },
};

function sn27Response() {
  return new Response(JSON.stringify(SN27_BODY), {
    status: 200,
    headers: { "content-type": "application/json" },
  });
}

describe("SN27 NI Compute call_subnet_surface verification (#7043)", () => {
  ...
  test("callSubnetSurface returns the real JSON body using the surface's own url + GET", async () => {
    ...
    assert.equal((result.body as Row).netuid, 27);
    assert.equal(typeof (result.body as Row).id, "string");
    assert.equal(typeof (result.body as Row).is_active, "boolean");
    assert.equal(typeof (result.body as Row).latest_snapshot, "object");
    assert.equal((result.body as Row).latest_snapshot.netuid, 27);
  });
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    ...
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 27 }],
    };
    const deps = {
    ...
    try {
      ...
      assert.equal(result.structuredContent.body.netuid, 27);
    } finally {
