...
//
// The surface is the public no-auth SN18 API health endpoint
// (sn-18-zeus-health, GET https://api.zeussubnet.com/v1/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/zeus.json", import.meta.url)),
    "utf8",
  ),
);
const SURFACE = registry.surfaces.find((surface) => surface.id === SURFACE_ID);

// A faithful subset of the live https://api.zeussubnet.com/v1/health response body.
...
describe("SN18 SN18 call_subnet_surface verification (#7034)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.zeussubnet.com/v1/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 18 }],
    };
    const deps = {
