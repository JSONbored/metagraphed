...
import {
  ...
  buildValidatorDetail as buildValidatorDetailRaw,
...
// row-array builders) a non-nullable `Row[]`/non-null first-argument type,
// but these tests dot repeatedly into the returned shape (data.validators[0].
// foo, etc.) and several also exercise a builder's documented cold-safe
// handling of a non-array/malformed first argument (a bare null, or an array
// containing null/undefined elements). Wrapping once here -- forwarding to
// the real implementation completely unchanged, only casting the input/
// output TYPES -- keeps every call site below exactly as originally written
// instead of casting at each of the ~100 call sites.
function formatNeuron(
  row: unknown,
  featuredHotkeys?: Set<string>,
  immunityPeriod?: number,
): Row {
  return formatNeuronRaw(
    row as Row | null | undefined,
    featuredHotkeys,
    immunityPeriod,
  ) as unknown as Row;
}
function buildSubnetMetagraph(
  rows: unknown,
  netuid: unknown,
  options?: Parameters<typeof buildSubnetMetagraphRaw>[2],
): Row {
  return buildSubnetMetagraphRaw(rows as Row[], netuid, options) as Row;
}
function buildSubnetValidators(
  rows: unknown,
  netuid: unknown,
  options?: Parameters<typeof buildSubnetValidatorsRaw>[2],
): Row {
...
function buildNeuronDetail(
  row: unknown,
  netuid: unknown,
  options?: Parameters<typeof buildNeuronDetailRaw>[2],
): Row {
  return buildNeuronDetailRaw(
    row as Row | null | undefined,
    netuid,
    options,
  ) as Row;
...
function composeValidatorComparison(
  details: unknown,
  options?: Parameters<typeof composeValidatorComparisonRaw>[1],
): Row {
  return composeValidatorComparisonRaw(details, options) as Row;
}
function overlayFeaturedValidators(data: unknown): Row {
...
describe("metagraph-neurons builders", () => {
  ...
  test("formatNeuron omits immunity_expires_at* when immunityPeriod is invalid", () => {
    for (const bad of [-1, 1.5, Number.NaN, "7200", null, undefined]) {
      const n = formatNeuron(IMMUNE_ROW, undefined, bad as number);
      assert.equal(
  ...
  test("buildGlobalValidators groups validator identities across subnets", () => {
    ...
    assert.deepEqual(
      top.subnets.map((s: Row) => [s.netuid, s.uid]),
      [
        [1, 2],
        [2, 1],
        [5, 3],
  ...
  test("buildGlobalValidators splits root (netuid 0) stake from alpha stake in exact rao space (#2550)", () => {
    ...
    const entry = data.validators.find((v: Row) => v.hotkey === "hk-root");
  ...
  test("buildGlobalValidators joins nominator_count by hotkey, nulls a hotkey with no row (#2549)", () => {
    ...
    const known = data.validators.find((v: Row) => v.hotkey === "hk-known");
    const unknown = data.validators.find((v: Row) => v.hotkey === "hk-unknown");
  ...
  test("buildGlobalValidators takes the first non-null take per hotkey and ignores later rows (#2548)", () => {
    ...
    const byHotkey = Object.fromEntries(
      data.validators.map((v: Row) => [v.hotkey, v.take]),
    );
  ...
  test("buildGlobalValidators sets `featured` per hotkey entry, defaulting false", () => {
    ...
    const byHotkey = Object.fromEntries(
      data.validators.map((v: Row) => [v.hotkey, v.featured]),
    );
  ...
  test("buildGlobalValidators uses deterministic footprint tie-breakers", () => {
    ...
    assert.deepEqual(
      data.validators[0].subnets.map((subnet: Row) => [
        subnet.netuid,
        subnet.uid,
      ]),
      [
    ...
    assert.deepEqual(
      alphabetical.validators.map((validator: Row) => validator.hotkey),
      ["hk-a", "hk-b"],
    );
  });
  ...
  test("buildValidatorDetail aggregates one hotkey's validator rows across every subnet", () => {
    ...
    assert.deepEqual(
      data.subnets.map((s: Row) => [s.netuid, s.uid]),
      [
        [1, 2],
        [1, 5],
        [2, 1],
  ...
  test("buildGlobalValidators rolls up stake/emission totals and stake dominance", () => {
    ...
    const heavy = data.validators.find((v: Row) => v.hotkey === "hk-heavy");
    const light = data.validators.find((v: Row) => v.hotkey === "hk-light");
    const bad = data.validators.find((v: Row) => v.hotkey === "hk-bad");
    ...
    assert.deepEqual(
      data.validators.map((v: Row) => v.hotkey),
  ...
  test("buildGlobalValidators nulls stake dominance when network stake is zero", () => {
    ...
    assert.equal(
      data.validators.every((v: Row) => v.stake_dominance === null),
      true,
    );
  });
  ...
  test("sums a hotkey's per-UID stake in exact rao space, not compounding float error (#2922)", () => {
    ...
    const entry = data.validators.find((v: Row) => v.hotkey === "hk-precision");
  ...
  test("buildGlobalValidators joins coldkey_identity by coldkey, not hotkey -- shared across two hotkeys (#5234)", () => {
    ...
    const byHotkey = Object.fromEntries(
      data.validators.map((v: Row) => [v.hotkey, v.coldkey_identity]),
    );
          ...
          return {
            all() {
              ...
              if (sql.includes("hotkey = ?")) {
                r = r.filter((x: Row) => x.hotkey === params[0]);
              }
