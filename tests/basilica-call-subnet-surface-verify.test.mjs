...
//
// The surface is the public no-auth SN39 API health endpoint
// (sn-39-basilica-subnet-api, GET https://api.basilica.ai/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/basilica.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.basilica.ai/health response body.
...
describe("SN39 Basilica call_subnet_surface verification (#7053)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.basilica.ai/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 39 }],
    };
    const deps = {
