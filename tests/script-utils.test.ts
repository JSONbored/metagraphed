...
describe("script utility contracts", () => {
  ...
  test("README link review rejects malformed and generic references", () => {
    assert.equal(
      isReviewableReadmeLink(
        null as unknown as Parameters<typeof isReviewableReadmeLink>[0],
      ),
      false,
    );
    assert.equal(
  ...
  test("augments manual overlays with verified baseline surfaces", async () => {
    ...
    const rejectedBaseline = overlaySet.manualBaselineOverlays[0].surfaces.find(
      (surface: Row) => surface.id === "sn-25-native-chain-website",
    );
    assert.equal(rejectedBaseline.url, "https://example.com/rejected");
    assert.equal(overlaySet.generatedOverlays.length, 0);
    assert.deepEqual(
      overlaySet.manualOverlays[0].surfaces.map((surface: Row) => surface.id),
  ...
  test("only elevates generated overlays when reviewed evidence is promoted", async () => {
    ...
    assert.deepEqual(
      overlaySet.generatedOverlays[0].surfaces.map(
  ...
  test("does not promote HTML-only Swagger pages as OpenAPI surfaces", async () => {
    ...
    assert.deepEqual(
      overlaySet.generatedOverlays[0].surfaces.map((surface: Row) => ({
        id: surface.id,
  ...
  test("resolves hostnames before treating probe URLs as safe", async () => {
    ...
    ]) as unknown as Parameters<typeof isUnsafeResolvedUrl>[1];
    const publicResolver = (async () => [
      { address: "93.184.216.34", family: 4 },
    ]) as unknown as Parameters<typeof isUnsafeResolvedUrl>[1];
    const emptyResolver = (async () => []) as unknown as Parameters<
      typeof isUnsafeResolvedUrl
    >[1];
  ...
  test("builds RPC endpoint and pool artifacts from surface health", () => {
    ...
    assert.equal(
      rpc.endpoints.find((endpoint: Row) => endpoint.id === "root-rpc")
        .health_source,
      "probe-derived",
    );
    assert.equal(
    ...
    assert.equal(
      rpc.endpoints.find((endpoint: Row) => endpoint.id === "root-rpc").last_ok,
      "1970-01-01T00:00:00.000Z",
    );
    assert.equal(
      rpc.endpoints.find((endpoint: Row) => endpoint.id === "root-failed-rpc")
        .health_stale,
      true,
    );
    ...
    const rootRpcEndpoint = endpointResources.endpoints.find(
      (endpoint: Row) => endpoint.surface_id === "root-rpc",
    );
    ...
    assert.equal(
      endpointResources.endpoints.find(
        (endpoint: Row) => endpoint.surface_id === "root-docs",
      ).layer,
      "docs-provider",
    );
    assert.equal(
      endpointResources.endpoints.find(
        (endpoint: Row) => endpoint.surface_id === "root-data",
      ).layer,
      "data-provider",
    );
    assert.equal(
    ...
    assert.equal(
      endpointResources.endpoints.find(
        (endpoint: Row) => endpoint.surface_id === "root-private-api",
      ).publication_state,
      "disabled",
    );
    assert.equal(
      endpointResources.endpoints
        .find((endpoint: Row) => endpoint.surface_id === "root-private-api")
    ...
    assert.equal(
      pools.pools.find((pool: Row) => pool.id === "finney-rpc").eligible_count,
      1,
    );
    assert.equal(
      pools.pools.find((pool: Row) => pool.id === "finney-archive")
        .eligible_count,
      1,
    );
    assert.equal(
      pools.pools.find((pool: Row) => pool.id === "finney-wss").eligible_count,
      0,
    );
    assert.equal(
      pools.pools.find((pool: Row) => pool.id === "finney-wss").endpoints[0]
        .score > 0,
      true,
    );
    ...
    assert.equal(
      pools.pools
        .find((pool: Row) => pool.id === "finney-rpc")
        .endpoints.every((endpoint: Row) =>
    ...
    assert.equal(
      pools.pools
        .find((pool: Row) => pool.id === "finney-rpc")
        .endpoints.find((endpoint: Row) => endpoint.id === "root-rpc")
        .health_source,
      "probe-derived",
    );
    assert.equal(
      pools.pools
        .find((pool: Row) => pool.id === "finney-rpc")
        .endpoints.find((endpoint: Row) => endpoint.id === "root-rpc").last_ok,
      "1970-01-01T00:00:00.000Z",
    );
    ...
    assert.equal(
      generalizedPools.provider_scores.some(
        (provider: Row) => provider.provider === "degraded",
      ),
      true,
    );
    assert.equal(
      generalizedPools.pools
        .find((pool: Row) => pool.id === "finney-rpc")
        .endpoints.find((endpoint: Row) => endpoint.surface_id === "root-rpc")
        .surface_key,
      rootRpcSurface.key,
    );
    ...
    const failedEndpoint = endpointResources.endpoints.find(
      (endpoint: Row) => endpoint.surface_id === "root-failed-rpc",
    );
    const degradedEndpoint = endpointResources.endpoints.find(
      (endpoint: Row) => endpoint.surface_id === "root-degraded-rpc",
    );
    ...
    assert.equal(
      endpointResources.endpoints
        .find((endpoint: Row) => endpoint.surface_id === "root-rpc")
        .score_reasons.find((reason: Row) => reason.reason === "method-support")
        .points,
      10, // object shape: 2 methods x 5
    );
    assert.equal(
      degradedEndpoint.score_reasons.find(
        (reason: Row) => reason.reason === "method-support",
      ).points,
    ...
    assert.equal(
      incidents.incidents.find(
        (incident: Row) => incident.endpoint_id === degradedEndpoint.id,
      ).severity,
      "warning",
    );
  ...
  test("endpoint resources keep stable ids across display slug renames", () => {
    const buildRenamedEndpoint = (surfaceId: string) =>
      buildEndpointResourceArtifact({
  ...
  test("every artifact size budget maps to a real build-artifacts write", async () => {
    ...
    const budgetToRegExp = (pattern: string) =>
      new RegExp(
        `^${pattern
          .replace(/[.+?^${}()|[\]\\]/g, "\\$&")
          .replace(/\*/g, "[^/]*")}$`,
...
describe("adapter gittensor master summary", () => {
  ...
  test("aggregates shares and ranks the top emission repositories", () => {
    ...
    assert.deepEqual(
      summary.top_emission_repositories.map((repo: Row) => repo.repository),
  ...
  test("treats a JSON null repo value as zero shares without throwing", () => {
    ...
    const nulled = summary.top_emission_repositories.find(
      (repo: Row) => repo.repository === "org/null",
    );
