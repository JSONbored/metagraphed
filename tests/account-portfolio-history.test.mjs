...
// Two snapshot days: on 2026-06-02 a validator seat on 7 and a miner seat on 12; on 2026-06-01
// only the miner seat on 12. Rows arrive newest-first (the loader's ORDER BY snapshot_date DESC).
const ROWS = [
  {
...
describe("buildAccountPortfolioHistory", () => {
  ...
  test("drops rows with a junk snapshot_date or netuid (incl. a negative number)", () => {
    const out = buildAccountPortfolioHistory(
      [
        ...
        { snapshot_date: "2026-06-02", netuid: "" }, // blank netuid → not a subnet
    ...
    assert.equal(out.points[0].position_count, 3); // the 3 valid-date rows kept
    assert.equal(out.points[0].subnet_count, 1); // only netuid 7 (blank/negative dropped)
  ...
  test("cold/empty → schema-stable empty series with the default window label", () => {
  ...
  test("loader flags capped when the read hits the row cap", async () => {
    ...
    const out = await loadAccountPortfolioHistory(async () => many, SS58, {
      windowLabel: "7d",
      windowDays: 7,
    });
...
describe("GET /api/v1/accounts/{ss58}/portfolio-history", () => {
  ...
  test("rejects an unknown query parameter with 400", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${SS58}/portfolio-history?foo=bar`,
      ),
      dailyEnv(ROWS),
      {},
    );
