...
//   sn-66-ninja-health  GET https://ninja66.ai/health
//     -> HTTP 200 text/plain body `ok` (not JSON)
// Issue #7079 lists this single surface only.
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/ninja.json", import.meta.url)),
    "utf8",
  ),
);
...
describe("SN66 ninja call_subnet_surface verification (#7079)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://ninja66.ai/health");
    assert.equal(SURFACE.schema_url, undefined);
  });

  test("callSubnetSurface returns the plain-text ok body via GET", async () => {
