...
const ADDR = "5GReferenceAccountAddressForRegistrationsTestsssss";

describe("buildAccountRegistrations", () => {
  test("cold / empty input yields a zeroed, schema-stable card", () => {
    for (const rows of [[], null, undefined]) {
      const d = buildAccountRegistrations(rows, ADDR, { window: "30d" });
      assert.equal(d.schema_version, 1);
      assert.equal(d.address, ADDR);
      assert.equal(d.window, "30d");
      assert.equal(d.total_registrations, 0);
  ...
  test("omitted window defaults to null", () => {
    assert.equal(buildAccountRegistrations([], ADDR).window, null);
  });

  test("folds per-subnet registration counts + first/last timestamps", () => {
    const d = buildAccountRegistrations(
      [
    ...
    assert.equal(d.total_registrations, 4);
    ...
    assert.equal(
      s1.last_registered_at,
      new Date(1_700_500_000_000).toISOString(),
    );
  });

  test("HHI concentration: all registrations on one subnet -> 1, spread -> < 1", () => {
    const one = buildAccountRegistrations([row(1, 5, 1000, 2000)], ADDR, {
      window: "7d",
    });
    assert.equal(one.concentration, 1);
    // 3 and 3 across two subnets: HHI = (9 + 9) / 36 = 0.5.
    const split = buildAccountRegistrations(
  ...
  test("never rounds a sub-perfect concentration up to exactly 1", () => {
    ...
    // 1.0000 at 4dp but is < 1 — the anti-overstatement clamp holds it at 0.9999 so a wallet
  ...
  test("ties on registration count break by netuid ascending", () => {
    const d = buildAccountRegistrations(
  ...
  test("merges duplicate netuid rows and keeps the widest first/last span", () => {
    ...
    const s = d.subnets[0];
    assert.equal(s.registrations, 3); // 2 + 1
    assert.equal(s.first_registered_at, new Date(1000).toISOString()); // min
    assert.equal(s.last_registered_at, new Date(5000).toISOString()); // max
  });
  ...
  test("skips malformed/blank/negative netuid and zero-count rows", () => {
    const d = buildAccountRegistrations(
      [
        row(1, 4, 1000, 2000),
        { netuid: null, registrations: 3 },
        { netuid: "", registrations: 3 },
        { netuid: "bad", registrations: 3 },
        { netuid: -1, registrations: 3 },
        row(2, 0, 1000, 2000), // zero registrations: skipped
      ],
      ADDR,
      { window: "7d" },
    );
...
describe("loadAccountRegistrations", () => {
  ...
  test("a cold store (no rows) yields a zeroed card + null generatedAt", async () => {
    ...
    assert.equal(data.total_registrations, 0);
