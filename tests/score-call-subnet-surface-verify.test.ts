...
//
// The surface is the public no-auth SN44 API health endpoint
// (sn-44-score-subnet-api, GET https://api.scorevision.io/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/score.json", import.meta.url)),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.scorevision.io/health response body.
...
describe("SN44 Score call_subnet_surface verification (#7058)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.scorevision.io/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 44 }],
    };
    const deps = {
