...
const added = (coldkey: unknown, tao: unknown, count = 1, at: unknown = 1000) =>
...
describe("buildValidatorNominators", () => {
  test("cold / empty input yields a zeroed, schema-stable card", () => {
    for (const rows of [[], null, undefined]) {
      const d = buildValidatorNominators(rows, HOTKEY, {
        window: "30d",
      }) as Row;
  ...
  test("omitted window defaults to null", () => {
    assert.equal((buildValidatorNominators([], HOTKEY) as Row).window, null);
  });
  ...
  test("folds per-(coldkey, kind) rows into per-nominator flow", () => {
    ...
    const a = d.nominators.find((n: Row) => n.coldkey === "ck-a");
  ...
  test("folds SQL-paginated per-coldkey aggregate rows", () => {
    ...
    assert.deepEqual(
      d.nominators.map((n: Row) => [n.coldkey, n.staked_tao, n.unstaked_tao]),
      [
        ["ck-a", 50, 0],
        ["ck-b", 0, 5],
      ],
  ...
  test("sorting by last_activity falls back to -Infinity for a nominator with no valid observed timestamp", () => {
    ...
    // or comparing equal.
    assert.deepEqual(
      d.nominators.map((n: Row) => n.coldkey),
  ...
  test("sorts by net_staked (default), gross_staked, or last_activity", () => {
    ...
    assert.deepEqual(
      byNet.nominators.map((n: Row) => n.coldkey),
    ...
    assert.deepEqual(
      byGross.nominators.map((n: Row) => n.coldkey),
      ["ck-big-net", "ck-small-net"],
    );
    ...
    assert.deepEqual(
      byActivity.nominators.map((n: Row) => n.coldkey),
  ...
  test("an unsupported sort falls back to the default", () => {
    assert.equal(
      (buildValidatorNominators([], HOTKEY, { sort: "bogus" }) as Row).sort,
      DEFAULT_NOMINATOR_SORT,
    );
  });
  ...
  test("limit clamps to [0, max] and paginates via offset; nominator_count reports the full set", () => {
    ...
    const zero = buildValidatorNominators(rows, HOTKEY, { limit: 0 }) as Row;
    ...
    assert.deepEqual(
      page.nominators.map((n: Row) => n.coldkey),
      ["ck-2", "ck-3"],
    );
  });
