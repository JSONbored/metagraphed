...
describe("overlaySubnetHealth", () => {
  test("builds per-subnet health from live rows without stale static surfaces", () => {
    ...
    const merged = overlaySubnetHealth(staticArtifact, liveCurrent, 7) as Row;
    const api = merged.surfaces.find((s: Row) => s.surface_id === "sn7-api");
    assert.equal(api.status, "ok"); // overlaid live
    assert.equal(api.observed_by, "live-cron-prober");
    assert.equal(
      merged.surfaces.some((s: Row) => s.surface_id === "sn7-docs"),
      false,
    );
  ...
  test("returns null with no live snapshot (caller falls back to static)", () => {
    assert.equal(overlaySubnetHealth({ surfaces: [] }, null, 7) as Row, null);
  });
});
...
describe("buildGlobalHealth", () => {
  test("serves the live operational summary when present", () => {
    ...
    const out = buildGlobalHealth(live, { contract_version: "v" }) as Row;
  ...
  test("returns null when cold so the caller serves static", () => {
    assert.equal(buildGlobalHealth(null, { subnets: [] }) as Row, null);
  });
});
...
describe("mergeRpcEndpoints", () => {
  ...
  test("folds a row with no status into 'unknown' when tallying by_status", () => {
    ...
    const merged = mergeRpcEndpoints(stat, { endpoints: [] }) as Row;
  ...
  test("folds unrecognized live status into unknown on the endpoint row", () => {
    ...
    const a = (mergeRpcEndpoints(stat, live) as Row).endpoints.find(
      (e: Row) => e.id === "a",
    );
  ...
  test("observed_at falls back to last_ok, then null, when the run time is absent", () => {
    ...
    const merged = (mergeRpcEndpoints(stat, live) as Row).endpoints;
    assert.equal(
      merged.find((e: Row) => e.id === "a").observed_at,
      "2026-06-10T08:00:00Z",
    );
    assert.equal(merged.find((e: Row) => e.id === "b").observed_at, null);
  });
});
...
describe("overlayRpcPoolEligibility", () => {
  ...
  test("drops endpoints only after sustained (>=2) consecutive failures", () => {
    ...
    const out = overlayRpcPoolEligibility(pool, live) as Row;
    assert.equal(
      out.endpoints.find((e: Row) => e.id === "a").pool_eligible,
      true,
    );
    assert.equal(
      out.endpoints.find((e: Row) => e.id === "b").pool_eligible,
      false,
    );
    assert.deepEqual(
      out.endpoints.find((e: Row) => e.id === "b").pool_eligibility_reasons,
      ["sustained-down"],
    );
  });
  ...
  test("folds unrecognized live status into unknown on the pool endpoint row", () => {
    ...
    const out = overlayRpcPoolEligibility(pool, live) as Row;
    assert.equal(
      out.endpoints.find((e: Row) => e.id === "a").status,
      "unknown",
    );
  });

  test("returns the static pool unchanged when live is cold", () => {
    assert.equal(overlayRpcPoolEligibility(pool, null) as Row, pool);
  });
  ...
  test("restores eligibility once a previously-degraded endpoint's live status recovers to ok", () => {
    ...
    const out = overlayRpcPoolEligibility(degradedPool, live) as Row;
    const a = out.endpoints.find((e: Row) => e.id === "a");
  ...
  test("structural policy (auth_required/public_safe/kind) stays disqualifying regardless of live health", () => {
    ...
    const out = overlayRpcPoolEligibility(authedPool, live) as Row;
    const a = out.endpoints.find((e: Row) => e.id === "a");
...
describe("mergeFreshness", () => {
  test("marks surface-health current + warn from live meta", () => {
    ...
    const sh = out.sources.find((s: Row) => s.id === "surface-health");
    ...
    assert.equal(
      out.sources.find((s: Row) => s.id === "native-subnets").stale_behavior,
      "block",
    );
...
describe("subnetBadgeStatus", () => {
  test("finds the subnet rollup", () => {
    ...
    assert.equal((subnetBadgeStatus(live, 7) as Row).status, "degraded");
    assert.equal(subnetBadgeStatus(live, 9) as Row, null);
  });
});

describe("summarizeRows / rollupStatus", () => {
  const row = (status: unknown, extra: Row = {}) => ({ status, ...extra });

  test("empty rows → unknown status, null aggregates", () => {
    const out = summarizeRows([]) as Row;
  ...
  test("all-unknown → unknown", () => {
    assert.equal(
      (summarizeRows([row("unknown"), row("unknown")]) as Row).status,
      "unknown",
    );
  });
  test("all-ok → ok", () => {
    assert.equal((summarizeRows([row("ok"), row("ok")]) as Row).status, "ok");
  });
  test("ok + failed mix → degraded", () => {
    assert.equal(
      (summarizeRows([row("ok"), row("failed")]) as Row).status,
      "degraded",
    );
  });
  test("ok + degraded mix → degraded", () => {
    assert.equal(
      (summarizeRows([row("ok"), row("degraded")]) as Row).status,
      "degraded",
    );
  });
  test("degraded + failed (no ok) → degraded (right-hand OR operand)", () => {
    ...
    assert.equal(
      (summarizeRows([row("degraded"), row("failed")]) as Row).status,
      "degraded",
    );
  });
  test("all-failed (no ok, no degraded) → failed", () => {
    const out = summarizeRows([row("failed"), row("failed")]) as Row;
    assert.equal(out.status, "failed");
    assert.equal(out.failed_count, 2);
  });
  test("unrecognized status values roll up as unknown, not ok", () => {
    const out = summarizeRows([row("weird"), row("weird")]) as Row;
    assert.equal(out.status, "unknown");
  ...
  test("null or missing status is treated as unknown", () => {
    const out = summarizeRows([row(null), { status: undefined }]) as Row;
    assert.equal(out.status, "unknown");
    assert.equal(out.unknown_count, 2);
  });
...
describe("overlaySubnetHealth (additional paths)", () => {
  test("null/empty live → null (no surfaces array)", () => {
    assert.equal(overlaySubnetHealth({ surfaces: [] }, null, 7) as Row, null);
    assert.equal(overlaySubnetHealth({ surfaces: [] }, {}, 7) as Row, null);
    assert.equal(
      overlaySubnetHealth({ surfaces: [] }, { surfaces: "nope" }, 7) as Row,
      null,
    );
  });
  ...
  test("no live rows for the netuid AND no static artifact → null", () => {
    ...
    assert.equal(overlaySubnetHealth(null, live, 7) as Row, null);
  });
  ...
  test("static null but live present → builds from live only", () => {
    ...
    const out = overlaySubnetHealth(null, live, 7) as Row;
    assert.equal(out.netuid, 7);
  ...
  test("static artifact without a surfaces array → treated as empty, live pushed", () => {
    ...
    const out = overlaySubnetHealth({ schema_version: 2 }, live, 7) as Row;
  ...
  test("live surfaces NOT in static get pushed as new operational surfaces", () => {
    ...
    const out = overlaySubnetHealth(staticArtifact, live, 7) as Row;
    assert.equal(out.contract_version, "cv");
    assert.equal(out.generated_at, "ga");
    assert.equal(out.slug, "acme");
    assert.equal(out.name, "Acme");
    const ids = out.surfaces.map((s: Row) => s.surface_id).sort();
    assert.deepEqual(ids, ["sn7-api", "sn7-new"]);
    const pushed = out.surfaces.find((s: Row) => s.surface_id === "sn7-new");
    assert.equal(pushed.observed_by, "live-cron-prober");
...
describe("buildGlobalHealth (additional paths)", () => {
  test("null live → null", () => {
    assert.equal(buildGlobalHealth(null, {}) as Row, null);
  });
  test("live without a summary → null", () => {
    assert.equal(buildGlobalHealth({ generated_at: "g" }, {}) as Row, null);
  });
...
describe("subnetBadgeStatus (additional paths)", () => {
  test("null live → null", () => {
    assert.equal(subnetBadgeStatus(null, 7) as Row, null);
  });
  test("live without subnets array → null", () => {
    assert.equal(subnetBadgeStatus({ subnets: "nope" }, 7) as Row, null);
  });
});

describe("mergeRpcEndpoints (additional paths)", () => {
  test("null live or live without endpoints array → null", () => {
    assert.equal(mergeRpcEndpoints({ endpoints: [] }, null) as Row, null);
    assert.equal(mergeRpcEndpoints({ endpoints: [] }, {}) as Row, null);
    assert.equal(
      mergeRpcEndpoints({ endpoints: [] }, { endpoints: "nope" }) as Row,
      null,
    );
  });
  ...
  test("archive_support falls back to the static value when live omits it", () => {
    ...
    const out = mergeRpcEndpoints(stat, live) as Row;
  ...
  test("static WITHOUT an endpoints array → null so caller serves static", () => {
    ...
    assert.equal(mergeRpcEndpoints({ schema_version: 1 }, live) as Row, null);
  });
  ...
  test("static null entirely → null so caller serves static", () => {
    ...
    assert.equal(mergeRpcEndpoints(null, live) as Row, null);
  });
});
...
describe("overlayRpcPoolEligibility (additional paths)", () => {
  test("null pool → returned unchanged (null)", () => {
    assert.equal(
      overlayRpcPoolEligibility(null, { endpoints: [] }) as Row,
      null,
    );
  });
  test("live without endpoints array → pool unchanged", () => {
    ...
    assert.equal(
      overlayRpcPoolEligibility(pool, { endpoints: "nope" }) as Row,
      pool,
    );
    assert.equal(overlayRpcPoolEligibility(pool, {}) as Row, pool);
  });
  ...
  test("endpoint with no live match stays unchanged; latency fallback used", () => {
    ...
    const noLive = out.endpoints.find((e: Row) => e.id === "no-live");
  ...
  test("pool without an endpoints array → maps over [] (no throw)", () => {
    const out = overlayRpcPoolEligibility(
      { id: "p" },
      { endpoints: [] },
    ) as Row;
...
describe("mergeFreshness (additional paths)", () => {
  test("null live meta or null static → null", () => {
    assert.equal(mergeFreshness({ sources: [] }, null) as Row, null);
    assert.equal(mergeFreshness(null, { last_run_at: "r" }) as Row, null);
  });

  test("sources NOT an array → passed through verbatim", () => {
    const stat = { sources: "nope", summary: { a: 1 } };
    const out = mergeFreshness(stat, { last_run_at: "r" }) as Row;
...
describe("formatTrends (additional paths)", () => {
  test("surfaces are sorted by surface_id; null avg_latency passes through", () => {
    ...
    assert.deepEqual(
      w.surfaces.map((s: Row) => s.surface_id),
      ["a", "m", "z"],
    );
    assert.equal(
      w.surfaces.find((s: Row) => s.surface_id === "z").avg_latency_ms,
      null,
    );
    assert.equal(
      w.surfaces.find((s: Row) => s.surface_id === "a").avg_latency_ms,
      13,
    );
    assert.equal(
      w.surfaces.find((s: Row) => s.surface_id === "m").uptime_ratio,
      null,
    );
  ...
  test("observedAt omitted → null", () => {
    const out = formatTrends({ netuid: 1, windows: { "7d": [] } }) as Row;
...
describe("formatBulkTrends", () => {
  test("groups daily rows by subnet and sorts subnets/points", () => {
    ...
    assert.deepEqual(
      out.windows["7d"].subnets.map((entry: Row) => entry.netuid),
      [7, 8],
    );
    ...
    assert.deepEqual(
      sn7.points.map((point: Row) => point.date),
  ...
  test("covers zero-sample and omitted-window fallback branches", () => {
    const empty = formatBulkTrends({}) as Row;
    ...
    } as unknown as Parameters<typeof formatBulkTrends>[0]) as Row;
...
function rpcPoolsArchiveFixture(artifact: Row) {
  return {
...
describe("worker live health serving", () => {
  ...
  test("/api/v1/rpc/pools overlays live KV health so a dead upstream is marked ineligible", async () => {
    ...
    const live = endpoints.find((e: Row) => e.id === "live");
    const dead = endpoints.find((e: Row) => e.id === "dead");
...
describe("resolveLiveHealth (KV → Postgres → null)", () => {
  ...
  test("rejects a stale KV health:current (wedged prober) and falls through", async () => {
    ...
    const live = (await resolveLiveHealth({
      ...
      env: mockEnv(), // no postgres tier → stale KV rejected → null (caller serves `unknown`)
    })) as Row;
...
describe("composed-artifact health overlays", () => {
  ...
  test("overlayOverviewHealth replaces baked health with live (or unknown)", () => {
    ...
    const out = overlayOverviewHealth(overview, live, 7) as Row;
    ...
    assert.equal(overlayOverviewHealth(overview, null, 7) as Row, null);
  });
  ...
  test("overlayCatalogDetail makes per-service health + callable live", () => {
    ...
    const out = overlayCatalogDetail(detail, live, 7) as Row;
    ...
    assert.equal(out.health_source, "live-cron-prober");
    assert.equal(overlayCatalogDetail(detail, null, 7) as Row, null);
  });
  ...
  test("overlayCatalogDetail gates readiness_verified on a live ok probe (#357)", () => {
    ...
    const dead = overlayCatalogDetail(detail, live, 7) as Row;
    ...
    const verified = overlayCatalogDetail(detail, okLive, 7) as Row;
  ...
  test("overlayArtifactEndpoints replaces baked per-endpoint health with live", () => {
    ...
    const out = overlayArtifactEndpoints(artifact, live) as Row;
  ...
  test("overlayArtifactEndpoints recomputes pool eligibility from live health and static constraints", () => {
    ...
    assert.equal(
      out.endpoints.find((e: Row) => e.surface_id === "rpc-ok").pool_eligible,
      true,
    );
    assert.deepEqual(
      out.endpoints.find((e: Row) => e.surface_id === "rpc-ok")
        .pool_eligibility_reasons,
      ["eligible"],
    );
    assert.equal(
      out.endpoints.find((e: Row) => e.surface_id === "api-ok").pool_eligible,
      false,
    );
    assert.deepEqual(
      out.endpoints.find((e: Row) => e.surface_id === "api-ok")
        .pool_eligibility_reasons,
      ["not-bittensor-base-layer"],
    );
    assert.equal(
      out.endpoints.find((e: Row) => e.surface_id === "auth-ok").pool_eligible,
      false,
    );
    assert.deepEqual(
      out.endpoints.find((e: Row) => e.surface_id === "auth-ok")
        .pool_eligibility_reasons,
      ["auth-required"],
    );
    assert.equal(
      out.endpoints.find((e: Row) => e.surface_id === "unsafe-ok")
        .pool_eligible,
      false,
    );
    assert.deepEqual(
  ...
  test("overlayArtifactEndpoints blanks endpoints to unknown when the store is cold", () => {
    ...
    const out = overlayArtifactEndpoints(artifact, null) as Row;
    assert.equal(out.endpoints[0].status, "unknown");
  ...
  test("overlayArtifactEndpoints returns null when there is no endpoints array", () => {
    assert.equal(overlayArtifactEndpoints({ netuid: 7 }, live) as Row, null);
    assert.equal(overlayArtifactEndpoints(null, live) as Row, null);
  });
...
describe("worker live health overlay on composed routes", () => {
  ...
  const seedDetailArchive = (detail: Row) => ({
    async get(key: string) {
...
describe("formatUptime (daily uptime history)", () => {
  ...
  test("returns an empty series + null reliability for no rows", () => {
    const out = formatUptime({ netuid: 7, window: "1y", rows: [] }) as Row;
...
describe("computeReliability (score from uptime history)", () => {
  ...
  test("latency penalty is bounded and only applies above 500ms", () => {
    ...
    assert.equal(
      scoreFromStats({ samples: 100, okCount: 100, avgLatencyMs: 99999 })!
        .score,
      85,
    );
    // no samples -> null
