...
//
// The surface is the public no-auth SN26 API health endpoint
// (sn-26-perturb-subnet-api, GET https://api.perturbai.io/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/perturb.json", import.meta.url)),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.perturbai.io/health response body.
...
describe("SN26 Perturb call_subnet_surface verification (#7042)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.perturbai.io/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 26 }],
    };
    const deps = {
