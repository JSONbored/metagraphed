...
describe("buildChainStakeTransferVolume", () => {
  ...
  test("skips malformed/blank/negative netuid, null/blank/non-numeric volume, and zero-transfer rows", () => {
    const data = buildChainStakeTransferVolume(
      [
        vrow(1, 40, 4),
        ...
        { netuid: 2, volume_tao: null, transfers: 1 }, // no summable volume (null)
        { netuid: 6, volume_tao: "   ", transfers: 1 }, // blank-string volume -> null
        { netuid: 8, volume_tao: "not-a-number", transfers: 1 }, // non-numeric volume -> null
        { netuid: 9, volume_tao: -50, transfers: 1 }, // negative aggregate -> malformed, skipped
  ...
  test("out-of-range and non-positive observed timestamps yield null, not a stamp or a RangeError", () => {
    // 1e100 is a finite but out-of-JS-Date-range epoch (getTime() -> NaN); 0 and a negative are
    // non-positive. All coerce to null rather than throwing or emitting a 1970 stamp.
    const data = buildChainStakeTransferVolume(
      [vrow(1, 40, 4, 1e100), vrow(2, 30, 2, 0), vrow(5, 25, 10, -1)],
      { window: "7d" },
    );
...
describe("loadChainStakeTransferVolume", () => {
  ...
  test("an omitted/garbled windowDays defensively falls back to the 7d default cutoff", async () => {
    let captured;
    const before = Date.now();
    await loadChainStakeTransferVolume(
      async (sql, params) => {
        captured = { sql, params };
        return [];
      },
    ...
    const after = Date.now();
    const dayMs = 24 * 60 * 60 * 1000;
    assert.equal(typeof captured.params[1], "number");
    assert.ok(Number.isFinite(captured.params[1]));
    assert.ok(captured.params[1] >= before - 7 * dayMs - 5);
    assert.ok(captured.params[1] <= after - 7 * dayMs + 5);
  });
