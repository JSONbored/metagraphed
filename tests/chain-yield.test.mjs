...
describe("buildChainYield", () => {
  ...
  test("coerces a numeric-epoch-string captured_at from D1 (not Date.parse NaN)", () => {
    ...
    // epoch string is NaN, which would drop the stamp to null. It must be read as
    // the real epoch-ms timestamp, matching the sibling chain readers.
    const out = buildChainYield([
  ...
  test("ignores a non-positive or overflowing numeric-string captured_at", () => {
    // "0" is epoch 0 (no real snapshot) and a giant digit string overflows to a
    // non-finite ms; both must drop to null rather than emit a bogus/throwing date.
    assert.equal(
      buildChainYield([{ stake_tao: 1, emission_tao: 0, captured_at: "0" }])
        .captured_at,
      null,
    );
    assert.equal(
      buildChainYield([
        { stake_tao: 1, emission_tao: 0, captured_at: "9".repeat(400) },
      ]).captured_at,
      null,
    );
  });
