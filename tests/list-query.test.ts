...
// Test-only cast: lets tests temporarily add/remove scratch collection keys
// (e.g. "__test_sparse_rows") on the const collection-config map without
// TypeScript's fixed-key literal object type rejecting the arbitrary index.
const queryCollections = API_QUERY_COLLECTIONS as Row;
...
function parseLink(value: string | null): Record<string, URL> {
...
function pageLink(path: string) {
  ...
  const { meta } = applyQueryFilters(data, url, "subnets") as Row;
...
describe("list-query pagination order", () => {
  ...
  test("order=desc without a sort key reports asc (rows are unsorted)", () => {
    ...
    assert.deepEqual(
      result.data.subnets.map((r: Row) => r.netuid),
      [3, 1, 2],
    );
...
describe("list-query sort with missing values", () => {
  ...
  const order = (result: Row) => result.data.subnets.map((r: Row) => r.netuid);
...
describe("list-query sort tie-break", () => {
  ...
  test("ties where rows lack netuid fall back to stable source order", () => {
    ...
    assert.deepEqual(
      result.data.subnets.map((r: Row) => r.id),
      ["c", "a", "b"],
    );
  });
});
...
describe("list-query curation curation_level filter (#6238)", () => {
  ...
  test("?curation_level=native narrows the curation list to matching rows", () => {
    ...
    assert.equal(result.error, undefined);
    assert.deepEqual(
      result.data.curation.map((r: Row) => r.netuid),
      [1],
    );
  });
...
describe("list-query unknown parameter validation (#2578)", () => {
  ...
  test("accepts format=csv on csv-enabled list routes", () => {
    ...
    assert.equal(result.error, undefined);
    assert.deepEqual(
      result.data.subnets.map((row: Row) => row.netuid),
      [1],
    );
  });
...
describe("list-query documents type/netuid (#6239)", () => {
  ...
  const ids = (result: Row) => result.data.documents.map((r: Row) => r.id);
  ...
  for (const type of ["subnet", "surface", "provider"]) {
    test(`?type=${type} filters to only that document type`, () => {
      ...
      assert.equal(result.error, undefined);
      assert.equal(
        result.data.documents.every((doc: Row) => doc.type === type),
        true,
      );
...
describe("list-query endpoint pool count range filters (#2587)", () => {
  ...
  const poolIds = (result: Row) => result.data.pools.map((r: Row) => r.id);
...
describe("list-query endpoint latency / score range filters (#2577)", () => {
  ...
  const surfaceIds = (result: Row) =>
    result.data.endpoints.map((r: Row) => r.surface_id);
...
describe("list-query pagination Link header", () => {
  ...
  test("a non-list (no pagination meta) collection yields no header", () => {
    ...
    assert.equal(
      paginationLinkHeader(
        query("/api/v1/subnets"),
        {} as unknown as Parameters<typeof paginationLinkHeader>[1],
      ),
      null,
    );
  });
...
describe("list-query string filter excludes rows missing the field", () => {
  ...
  function withCollection(fn: () => void) {
    const previous = queryCollections[collection];
  ...
  const ids = (result: Row) => result.data.rows.map((row: Row) => row.id);
...
describe("candidates id/confidence filters (#6242)", () => {
  ...
  const ids = (result: Row) => result.data.candidates.map((r: Row) => r.id);
  ...
  test("?confidence=<low|medium|high> filters to rows with that confidence", () => {
    ...
    assert.ok(
      result.data.candidates.every((row: Row) => row.confidence === "medium"),
    );
  });
...
describe("curated-surfaces id filter (#6242)", () => {
  ...
  const ids = (result: Row) => result.data.surfaces.map((r: Row) => r.id);
