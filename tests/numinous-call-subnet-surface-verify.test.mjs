...
//
// The surface is the public no-auth SN6 API health endpoint
// (sn-6-numinous-api-health, GET https://api.numinouslabs.io/api/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/numinous.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.numinouslabs.io/api/health response body.
...
describe("SN6 Numinous call_subnet_surface verification (#7022)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.numinouslabs.io/api/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 6 }],
    };
    const deps = {
