...
describe("MCP get_economics_trends", () => {
  ...
  test("rolls per-subnet snapshots into one network point per day from D1", async () => {
    ...
    const env = trendsD1(
      [
        // The newest day carries two subnets, so the tool exercises the
        // stake-weighted + median price aggregation, not just a passthrough.
        {
    ...
    const [newest] = out.days;
    assert.equal(newest.snapshot_date, "2026-06-02"); // newest first
    assert.equal(newest.subnet_count, 2);
    assert.equal(newest.total_stake_tao, 400); // 300 + 100
    // (0.02·300 + 0.06·100)/400 = 0.03 stake-weighted; median([0.02,0.06]) = 0.04.
    assert.equal(newest.alpha_price_tao_weighted, 0.03);
    assert.equal(newest.alpha_price_tao_median, 0.04);
    assert.equal(newest.validator_count, 10); // 8 + 2 summed across subnets
