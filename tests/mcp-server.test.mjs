...
describe("MCP stake-flow and movers economics tools", () => {
  ...
  function accountStakeTransfersD1(rows = [], capture = []) {
    return {
      METAGRAPH_HEALTH_DB: {
        prepare(sql) {
          return {
  ...
  test("get_account_stake_transfers shapes per-subnet transfer counts and concentration", async () => {
    ...
    try {
      ...
      assert.equal(
        out.subnets[0].first_transferred_at,
        new Date(1_717_000_000_000).toISOString(),
      );
      assert.ok(out.concentration > 0.5);
    } finally {
  ...
  test("get_account_stake_transfers rejects a missing ss58", async () => {
  ...
  test("get_account_stake_transfers payload validates against its declared outputSchema", async () => {
    const schema = listToolDefinitions().find(
      (t) => t.name === "get_account_stake_transfers",
    )?.outputSchema;
    const res = await callTool(
      "get_account_stake_transfers",
      { ss58: SS58 },
  ...
  test("stake-flow and movers payloads validate against outputSchemas", async () => {
    ...
    try {
      ...
      const accountTransfers = await callTool(
        "get_account_stake_transfers",
        { ss58: SS58 },
        { env: accountStakeTransfersD1([]) },
      );
      assert.ok(
        validatorFor("get_account_stake_transfers")(
          accountTransfers.body.result.structuredContent,
        ),
      );
