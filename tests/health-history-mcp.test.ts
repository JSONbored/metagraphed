...
function makeDeps({
  blob = HISTORY_BLOB,
  artifact = blob as Row | null,
}: { blob?: typeof HISTORY_BLOB; artifact?: Row | null } = {}) {
  return {
...
describe("health-history-mcp — loadHealthHistory", () => {
  ...
  test("applies list-query filters over a dated snapshot", async () => {
    ...
    assert.equal(typeof (out.summary as Row).incident_count, "number");
  });
...
describe("health-history-mcp — MCP metadata", () => {
  ...
  test("MCP server exports wire get_health_history", () => {
    ...
    const tool = MCP_TOOLS.find((t: Row) => t.name === "get_health_history");
