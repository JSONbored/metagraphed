...
// netuid 1: 4 hotkeys, 40 removals -> 10 removals/remover.
// netuid 2: 2 hotkeys, 30 removals -> 15 removals/remover.
// netuid 5: 10 hotkeys, 25 removals -> 2.5 removals/remover.
const SUBNETS = [rrow(1, 4, 40), rrow(2, 2, 30), rrow(5, 10, 25)];
// True network distinct hotkeys (12) is below the per-subnet sum (16): some removers remove an
// axon on more than one subnet and count once network-wide.
const NETWORK = {
  ...
  test("skips rows with a malformed/blank/negative netuid and zero-remover rows", () => {
    const data = buildChainAxonRemovals(
      [
        ...
        rrow(2, 0, 10), // zero removers: not a removal surface
      ],
      { window: "7d", networkDistinct: NETWORK },
    );
    assert.equal(data.subnet_count, 1);
