...
function arow(netuid, distinct_removers, removals) {
  return { netuid, distinct_removers, removals };
}

// netuid 1: 4 removers, 40 removals -> 10 removals/remover.
// netuid 2: 2 removers, 30 removals -> 15 removals/remover.
// netuid 5: 10 removers, 25 removals -> 2.5 removals/remover.
const SUBNETS = [arow(1, 4, 40), arow(2, 2, 30), arow(5, 10, 25)];
// True network distinct removers (12) is below the per-subnet sum (16): some removers remove an axon
...
describe("buildChainAxonRemovals", () => {
  ...
  test("rolls up the true distinct remover count and derived total events", () => {
  ...
  test("ties on total events break by netuid ascending", () => {
    const data = buildChainAxonRemovals([arow(9, 3, 50), arow(4, 2, 50)], {
      window: "7d",
      networkDistinct: NETWORK,
    });
    assert.deepEqual(
  ...
  test("merges duplicate netuid rows (sum removers and removals)", () => {
    const data = buildChainAxonRemovals([arow(1, 3, 20), arow(1, 2, 15)], {
      window: "7d",
      networkDistinct: NETWORK,
    });
  ...
  test("coerces non-numeric count cells to zero", () => {
    ...
    assert.equal(data.subnets[0].removals_per_remover, 0); // 0 removals / 3 removers
  });
  ...
  test("skips rows with a malformed/blank/negative netuid and zero-remover rows", () => {
    const data = buildChainAxonRemovals(
      [
        ...
        arow(2, 0, 10), // zero removers: not an axon-removal surface
      ],
      { window: "7d", networkDistinct: NETWORK },
    );
    assert.equal(data.subnet_count, 1);
