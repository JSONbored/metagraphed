...
describe("isReviewableReadmeLink", () => {
  test("rejects links missing a url or classification kind", () => {
    assert.equal(isReviewableReadmeLink(null as unknown as Row), false);
    assert.equal(
...
function link(url: string, kind: string) {
...
describe("selectReviewableReadmeLinks", () => {
  test("nullish links list yields an empty selection", () => {
    assert.deepEqual(selectReviewableReadmeLinks(null as unknown as Row[]), []);
