...
//
// The surface is the public no-auth SN24 indexed subnet feed on TaoMarketCap
// (sn-24-taomarketcap-subnet-api, GET
// https://api.taomarketcap.com/public/v1/subnets/24/, JSON, single fixed
// endpoint -- no schema). Live-verified 2026-07-21 to return HTTP 200
// application/json -- an object self-reporting netuid 24 (id, is_active,
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/quasar.json", import.meta.url)),
    "utf8",
  ),
);
const SURFACE = registry.surfaces.find((surface) => surface.id === SURFACE_ID);

// A faithful subset of the live SN24 TaoMarketCap subnet feed.
const BODY = {
  id: "24",
  netuid: 24,
  is_active: true,
  mechanism_count: 1,
  latest_snapshot_id: "8667931-24",
};
...
describe("SN24 Quasar call_subnet_surface verification (#7058)", () => {
