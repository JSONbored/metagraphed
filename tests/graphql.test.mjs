...
describe("graphql — subnet_gaps / subnet_evidence (#6980, baked review artifacts)", () => {
  ...
  test("subnet_gaps filters by review_state and pages with limit (#7880)", async () => {
    const env = fixtureEnv({
      "/metagraph/review/gaps/5.json": SUBNET_GAPS_BLOB,
    });
    const { status, body } = await gql(
      '{ subnet_gaps(netuid: 5, review_state: "community-submitted", limit: 1, order: "asc") }',
      env,
    );
    assert.equal(status, 200);
    assert.equal(body.errors, undefined);
    assert.equal(body.data.subnet_gaps.total, 1);
    assert.equal(body.data.subnet_gaps.returned, 1);
    assert.equal(
      body.data.subnet_gaps.priorities[0].review_state,
      "community-submitted",
    );
  });
