...
//
// The surface is the public no-auth SN37 API health endpoint
// (sn-37-aurelius-subnet-api, GET https://new-collector-api-production.up.railway.app/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/aurelius.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://new-collector-api-production.up.railway.app/health response body.
...
describe("SN37 Aurelius call_subnet_surface verification (#7052)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(
      SURFACE.url,
      "https://new-collector-api-production.up.railway.app/health",
    );
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 37 }],
    };
    const deps = {
