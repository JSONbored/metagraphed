...
//
// The surface is the public no-auth SN68 API endpoint (sn-68-nova-score-share-molecules, GET https://vali-score-share-api.metanova-labs.ai/api/v1/molecules,
// JSON, single fixed endpoint -- no schema). Live-verified 2026-07-21 to return
// HTTP 200 application/json with a top-level "items" field. The fixture below
// mirrors that live response's shape (live data, so the test pins the stable
// shape -- the "items" key is present and the body parses -- not exact values).
...
describe("SN68 SN68 call_subnet_surface verification (#7081)", () => {
  ...
  test("callSubnetSurface returns the real JSON body using the surface's own url + GET", async () => {
    ...
    assert.ok("items" in (result.body as Row));
  });

  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 68 }],
    };
    const deps = {
    ...
    try {
      ...
      assert.ok("items" in result.structuredContent.body);
    } finally {
