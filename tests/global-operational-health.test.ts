...
type LoadOpts = Parameters<typeof loadGlobalOperationalHealth>[1];
...
function readHealthKv(_env: unknown, key: string) {
...
describe("global-operational-health", () => {
  ...
  test("loadGlobalOperationalHealth builds the live global rollup from KV", async () => {
    const out = (await loadGlobalOperationalHealth(
      { env: {}, readHealthKv } as unknown as LoadCtx,
      { contractVersion: () => 99 } as LoadOpts,
    )) as Row;
    assert.equal(out.scope, "operational");
  ...
  test("accepts a static contractVersion without calling it as a function", async () => {
    const out = (await loadGlobalOperationalHealth(
      { env: {}, readHealthKv: async () => null } as unknown as LoadCtx,
      { contractVersion: 77 } as LoadOpts,
    )) as Row;
  ...
  test("falls back to unknown when live KV lacks a summary block", async () => {
    const out = (await loadGlobalOperationalHealth(
      {
        env: {},
        readHealthKv: async (_env: unknown, key: string) =>
          key === "health:current"
            ? { generated_at: "2026-06-11T00:00:00.000Z" }
            : null,
      } as unknown as LoadCtx,
      { contractVersion: () => 5 } as LoadOpts,
    )) as Row;
    assert.equal(out.health_source, "unavailable");
  ...
  test("MCP server exports wire get_network_health", () => {
    ...
    const tool = MCP_TOOLS.find((t: Row) => t.name === "get_network_health");
