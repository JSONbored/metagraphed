...
type GapsDeps = Parameters<typeof loadGapsList>[2];
...
describe("gaps-mcp", () => {
  ...
  test("loadGapsList returns filtered rows with pagination meta", async () => {
    const out = await loadGapsList(
      { env: {}, readArtifact } as unknown as GapsCtx,
      { netuid: 7 },
    );
  ...
  test("loadGapsList rejects invalid list-query params from REST parity", async () => {
    await assert.rejects(
      () =>
        loadGapsList({ env: {}, readArtifact } as unknown as GapsCtx, {
          fields: "not_a_column",
        }),
      (err: Row) => err.code === "invalid_params",
    );
