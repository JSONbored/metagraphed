...
//
// The surface is the public no-auth SN15 API health endpoint
// (sn-15-oro-health, GET https://api.oroagents.com/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../registry/subnets/oro.json", import.meta.url)),
    "utf8",
  ),
);
...
// A faithful subset of the live https://api.oroagents.com/health response body.
...
describe("SN15 SN15 call_subnet_surface verification (#7031)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://api.oroagents.com/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 15 }],
    };
    const deps = {
