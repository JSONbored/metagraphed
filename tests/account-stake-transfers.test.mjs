...
function row(netuid, transfers, first, last) {
  return { netuid, transfers, first_observed: first, last_observed: last };
}

const ADDR = "5G9hfkx9wGB1CLMT9WXkpHSAiYzjZb5o1Boyq4KAdDhjwrc5";
...
describe("GET /api/v1/accounts/{ss58}/stake-transfers", () => {
  function eventsEnv(rows) {
    return {
  ...
  test("returns the per-subnet stake-transfer footprint", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${ADDR}/stake-transfers?window=30d`,
      ),
      eventsEnv([row(1, 3, 1_700_000_000_000, 1_700_500_000_000)]),
      {},
    );
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.data.window, "30d");
    assert.equal(body.data.total_transfers, 3);
    assert.equal(body.data.subnets[0].netuid, 1);
    assert.equal(
      body.meta.artifact_path,
      `/metagraph/accounts/${ADDR}/stake-transfers.json`,
    );
  });
  ...
  test("defaults to the 30d window when omitted", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${ADDR}/stake-transfers`,
      ),
      eventsEnv([]),
      {},
    );
    assert.equal(res.status, 200);
    assert.equal((await res.json()).data.window, "30d");
  });

  test("rejects an unknown query parameter with 400", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${ADDR}/stake-transfers?bogus=1`,
      ),
      eventsEnv([]),
      {},
    );
  ...
  test("rejects an unsupported window with 400", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${ADDR}/stake-transfers?window=1y`,
      ),
      eventsEnv([]),
      {},
    );
    assert.equal(res.status, 400);
    assert.equal((await res.json()).meta.parameter, "window");
  });

  test("cold store → 200 with a zeroed card", async () => {
    const res = await handleRequest(
      new Request(
        `https://api.metagraph.sh/api/v1/accounts/${ADDR}/stake-transfers`,
      ),
