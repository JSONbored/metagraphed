...
type LoadDeps = Parameters<typeof loadRegistryCoverage>[1];
...
describe("registry-coverage", () => {
  ...
  test("loadRegistryCoverage returns the baked artifact payload", async () => {
    ...
    const out = (await loadRegistryCoverage(ctx)) as Row;
  ...
  test("loadRegistryCoverage maps artifact_not_found to not_found", async () => {
    ...
    await assert.rejects(
      ...
      (err: Row) => err.code === "not_found" && err.toolError === true,
    );
  });
  ...
  test("MCP server exports wire get_coverage", () => {
    ...
    const tool = MCP_TOOLS.find((t: Row) => t.name === "get_coverage");
    assert.ok(tool);
