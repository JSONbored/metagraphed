...
type DispatchArgs = Parameters<typeof dispatchWithRedelivery>[0];
...
describe("summarizeDeliveryRecords", () => {
  ...
  test("no/empty/nullish records → ok", () => {
    ...
    assert.deepEqual(summarizeDeliveryRecords(undefined), okSummary);
  ...
  test("pending records (no dead letter) → retrying", () => {
    ...
    assert.equal((summary.last_failure as Row).attempts, 2);
  });
  ...
  test("ignores malformed entries and reports the latest failure + dead count", () => {
    ...
    assert.equal((summary.last_failure as Row).event_id, "deadbeef");
    assert.equal((summary.last_failure as Row).attempts, 8);
  });
});
...
describe("dispatchWithRedelivery", () => {
  ...
  test("Phase 1 re-park continues the round even when the parked snapshot is truncated by redeliveryListLimit", async () => {
    ...
    const map = new Map<string, string>();
    const limitStore: Row = {
      async listKeys(prefix: string, opts: { limit?: number } = {}) {
  ...
  test("redelivers a previously-failed event on a later run with the SAME idempotency key, then clears it", async () => {
    store = makeStore();
    let firstHeaders: Row | undefined;
    await run({
      event: event7,
      now: () => T0,
      fetchFn: async (_url: unknown, init?: RequestInit) => {
        firstHeaders = init!.headers as Row;
    ...
    let secondHeaders: Row | undefined;
    let secondBody;
    const { redelivered } = await run({
      event: event9,
      now: () => "2026-06-22T00:00:05.000Z",
      fetchFn: async (_url: unknown, init?: RequestInit) => {
        secondHeaders = init!.headers as Row;
  ...
  test("an unsafe DNS result on the sweep drops the parked record", async () => {
    ...
    const { redelivered } = await run({
      ...
      resolveHostnames: async () => {
        ...
        (error as Row).code = "UNSAFE_WEBHOOK_DNS_RESULT";
        throw error;
      },
    });
    assert.equal(redelivered.length, 1);
  ...
  test("dispatchChangeEvent with no subscriptions → empty", async () => {
    assert.deepEqual(
      await dispatchChangeEvent({
        event: event7,
        fetchFn: ok200,
      } as unknown as Parameters<typeof dispatchChangeEvent>[0]),
      [],
    );
  });
});
