...
describe("SN128 ByteLeap call_subnet_surface verification (#7136)", () => {
  ...
  test("callSubnetSurface returns the real JSON body using the surface's own url + GET", async () => {
    ...
    assert.equal((result.body as Row).id, "128");
    assert.equal((result.body as Row).netuid, 128);
    assert.equal((result.body as Row).is_active, true);
    assert.equal((result.body as Row).latest_snapshot.netuid, 128);
    assert.equal(
      (result.body as Row).latest_snapshot.subnet_identities_v3.subnetName,
      "ByteLeap",
    );
  });
