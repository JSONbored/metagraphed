...
//
// The surface is the public no-auth SN71 API health endpoint
// (sn-71-leadpoet-health-api, GET https://leadpoet.com/api/health, JSON, single fixed endpoint -- no schema). Live-verified
...
const registry = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../registry/subnets/leadpoet.json", import.meta.url),
    ),
    "utf8",
  ),
);
...
// A faithful subset of the live https://leadpoet.com/api/health response body.
...
describe("SN71 SN71 call_subnet_surface verification (#7084)", () => {
  test("the registry surface exists and is configured to be callable", () => {
    ...
    assert.equal(SURFACE.url, "https://leadpoet.com/api/health");
  ...
  test("end-to-end through the call_subnet_surface MCP tool, resolved by surface id", async () => {
    const catalog = {
      surfaces: [{ ...SURFACE, surface_id: SURFACE.id, netuid: 71 }],
    };
    const deps = {
