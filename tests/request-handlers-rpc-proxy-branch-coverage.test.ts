...
const ep = (id: string, endpointUrl: string, extra: Row = {}) => ({
  id,
  url: endpointUrl,
  provider: "fixture",
  pool_eligible: true,
...
function rpcEnv(pool: Row, overrides: Record<string, unknown> = {}): Env {
  return {
...
function scriptedFetch(...replies: unknown[]) {
  const calls: unknown[] = [];
  const fn = async (target: unknown) => {
    const reply = replies[calls.length];
    calls.push(target);
    if (typeof reply === "function") return (reply as AnyFn)();
    return reply;
  };
  (fn as Row).calls = calls;
  return fn as typeof fn & Row;
}
...
function fakeCache() {
  ...
  return {
    ...
    async put(key: Row, response: Row) {
...
describe("handleRpcProxyRequest telemetry + cache path", () => {
  ...
  test("caches a successful quasi-static result and rebuilds the id on a hit", async () => {
    ...
    try {
      ...
      const idlessBody = (await idless.json()) as Row;
      assert.equal(
...
describe("state-query methods (#4344/9.2)", () => {
  ...
  test("forwards a well-formed state_getStorage at to upstream", async () => {
    ...
    globalThis.fetch = (async (_endpointUrl: unknown, init: RequestInit) => {
...
describe("handleSurfaceVerify alias + cache branches", () => {
  ...
  function verifyEnv(aliasOk: boolean): Env {
    return {
      ASSETS: {
        async fetch(request: Request) {
          const target = new URL(request.url);
