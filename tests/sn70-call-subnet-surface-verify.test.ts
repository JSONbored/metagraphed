...
async function callToolWithSurface(surface: Row) {
  const catalog = {
...
describe("SN70 NexisGen call_subnet_surface verification (#7083)", () => {
  describe("sn-70-nexisgen-validator-api-healthz", () => {
    ...
    test("callSubnetSurface returns the live 502 JSON body using the surface's own url + GET", async () => {
      ...
      assert.equal((result.body as Row).status, 502);
      assert.equal((result.body as Row).error_name, "origin_bad_gateway");
      assert.equal((result.body as Row).cloudflare_error, true);
      assert.equal((result.body as Row).retryable, true);
    });
  ...
  describe("sn-70-nexisgen-openapi (direct-call only)", () => {
    ...
    test("callSubnetSurface returns the live 502 JSON body using the surface's own url + GET", async () => {
      ...
      assert.equal(result.content_type, CONTENT_TYPE);
      assert.equal(result.truncated, false);
      assert.equal((result.body as Row).title, "Error 502: Bad gateway");
      assert.equal((result.body as Row).error_code, 502);
      assert.equal((result.body as Row).cloudflare_error, true);
    });
  });
});
