...
async function rpc(
  payload: unknown,
  {
    ...
    method = "POST",
    headers,
  }: { deps?: Row; env?: Row; method?: string; headers?: Row } = {},
) {
...
function aiEnvWithMatches(
  netuids: number[],
  { embedError, vectorError }: { embedError?: Row; vectorError?: Row } = {},
) {
  return {
...
describe("get_best_rpc_endpoint — dedupe, scoring + field fallbacks", () => {
  test("dedupes by id keeping the best score and maps absent fields to null", async () => {
    ...
    const b = out.endpoints.find((e: Row) => e.id === "b");
