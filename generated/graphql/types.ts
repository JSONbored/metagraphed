...
export type Query = {
  ...
  /** The largest TAO holders ranked by the chosen sort (total_tao by default), limit 1-100 (default 20). An unknown sort is a BAD_USER_INPUT error. Resolves to a schema-stable empty list when the holders tier is cold, never null. Recomputed every 6 hours (#9469) from the account_balances chain-state scan, per-subnet-priced stake positions, and the account_events stake stream, so captured_at advances with each pass and trails the present by up to one cycle. A null net_flow_* means no movement recorded in that window, not zero movement. Opaque JSON, matching the get_top_holders MCP/REST shape. Mirrors GET /api/v1/accounts/top-holders. */
