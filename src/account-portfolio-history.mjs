// A wallet's portfolio timeline: the daily trajectory of one hotkey's total stake, emission,
// footprint (subnet/position/validator counts) and overall return, rolled up per snapshot_date
// from the neuron_daily tier. The point-in-time companion to /accounts/{ss58}/portfolio (the
// current cross-subnet card): this answers "is this wallet's stake/emission/reach growing or
// shrinking?" over a 7d/30d/90d window. Distinct from /accounts/{ss58}/history (the raw event
// feed) and /accounts/{ss58}/stake-flow (net capital movement) — those track individual actions
// and flows; this tracks the standing balance each day. Pure + exported for unit tests; the
// Worker does the windowed D1 read + envelope. Null-safe: no rows -> schema-stable empty series.
...
// One hotkey over 90 days spans few subnets (a validator on ~40 subnets × 90d ≈ 3.6k rows); this
// cap is a safety valve on a pathological wallet, far above any real footprint. The builder drops
// a truncated oldest day so every point is whole.
...
// A snapshot_date cell must be a real YYYY-MM-DD string; anything else is dropped so a junk row
// can't open a bogus day bucket.
...
// Fold one day's neuron_daily rows into a single portfolio point. Distinct netuids give the subnet
// footprint; each row is a position; validator_permit=1 marks a validator seat. Emission/stake are
// summed across the wallet's seats.
...
// Group one hotkey's neuron_daily rows into a per-day portfolio series. Rows are read newest-first;
// points come back newest-first too. When the read hit the cap the oldest day may be partial, so
// it's dropped (capped=true) to keep every returned point whole. Null-safe: junk/sparse rows yield
// a schema-stable series.
...
// Shared D1 loader (REST + MCP parity): read this hotkey's neuron_daily rows within the window,
// newest-first, capped, and shape the timeline. The window's inclusive lower bound (the oldest
// in-window snapshot_date) is derived here so both surfaces share one cutoff. Cold/absent ->
// empty series.
