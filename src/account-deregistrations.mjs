// Per-account deregistration footprint: which subnets one account (hotkey) was deregistered from
// over a recent window, broken down per subnet and rolled up into a deregistration scorecard. Pure
// shaping (buildAccountDeregistrations) + a thin D1 loader (loadAccountDeregistrations); the Worker
// adds the REST envelope. Null-safe: a cold store or an empty window yields schema-stable zeros
// (never throws), matching the sibling account tiers (stake-flow, registrations, counterparties).
//
// This is the account-level companion of the per-subnet and network deregistration leaderboards
// (/api/v1/subnets/{netuid}/deregistrations and /api/v1/chain/deregistrations): those answer "who
// was deregistered from subnet N" / "which subnets saw the most deregistrations", this answers
// "which subnets was THIS account deregistered from, how often, and when" — a per-subnet
// NeuronDeregistered count with the first/last deregistration timestamps, an HHI concentration of
// where its deregistration activity is focused, and the dominant subnet. Distinct from
// /accounts/{ss58}/subnets (current registration STATE) and /accounts/{ss58}/registrations
// (windowed NeuronRegistered events) — this is windowed NeuronDeregistered churn, the exit-side
// counterpart to that entry-side event stream.

const DAY_MS = 24 * 60 * 60 * 1000;

// The account_events kind emitted when a neuron is deregistered from a subnet; always carries the
// deregistered hotkey (scripts/fetch-events.py _deregistered -> [netuid, uid, hotkey]).
...
// deregistered from two or more subnets (HHI < 1) must never render as 1, which this card's
...
export function buildAccountDeregistrations(rows, address, { window } = {}) {
  ...
  for (const row of list) {
    ...
    if (deregistrations === 0) continue; // no deregistrations on this subnet: skip
  ...
  // Most-deregistered subnets first, tie-broken by netuid for a stable, deterministic order.
  subnets.sort(
...
// seek. Returns { data, generatedAt } where generatedAt is the newest deregistration's observed_at
// as an ISO string (string|null per the envelope contract). Cold/absent D1 -> zeroed card + null.
