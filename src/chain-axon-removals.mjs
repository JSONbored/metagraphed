...
// per-subnet leaderboard plus a network rollup and intensity distribution. Raw axon-teardown
// activity across every subnet — the removal-side companion to the AxonServed announcements in
// /chain/serving (which counts axon announcements, not teardowns), the same split as
// /chain/registrations vs /chain/deregistrations. Pure shaping (buildChainAxonRemovals) + a thin D1
// loader (loadChainAxonRemovals); the field semantics live in
// schemas/components/05-subnets.schema.json (ChainAxonRemovalsArtifact).
...
// Round a removals-per-remover ratio to a stable precision (2dp). Always finite and non-negative
// here (events / distinct removers, with the divisor guarded below).
...
// Spread of the per-subnet re-teardown intensity across every subnet with axon-removal activity:
// count, mean, and min / p25 / median / p75 / p90 / max. Null when no subnet saw an axon removal.
...
// distinct_removers) plus the newest observed_at. `limit` caps the leaderboard; subnet_count and the
// distribution span every subnet with observed axon-removal activity (subnets with no
...
export function buildChainAxonRemovals(
  ...
  for (const row of list) {
    ...
    if (removers === 0) continue; // no hotkeys removed an axon: not an axon-removal surface
  ...
  // Most active axon-removing subnets first (by total AxonInfoRemoved events), tie-broken by netuid.
...
// (true distinct removers + newest observed_at) and then grouped by netuid for the per-subnet
// leaderboard, and shape with buildChainAxonRemovals. The newest-observed probe doubles as the
// cold-store guard: a null MAX(observed_at) skips the per-subnet read. The handler resolves
// windowLabel/windowDays from analyticsWindow (7d/30d). Cold/absent store -> the schema-stable
// empty block.
