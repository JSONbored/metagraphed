// Live network-wide axon-removal (teardown) activity from the account_events AxonInfoRemoved
...
// schemas/components/05-subnets.schema.json (ChainAxonRemovalsArtifact). The removal-side companion
// to the announcement-side /chain/serving: AxonInfoRemoved is a neuron's previously-announced axon
// endpoint being torn down, so it measures teardown, not the announcement itself. Ranked and counted
// by the removing hotkey, exactly the way subnet-axon-removals.mjs scopes a single subnet.
...
// non-negative here (events / distinct removers, with the divisor guarded below).
...
// re-announcing). A subnet with no removers has no defined intensity (null) rather than a
// divide-by-zero.
...
// Spread of the per-subnet re-teardown intensity (AxonInfoRemoved events per remover) across every
// subnet that saw a removal in the window: count, mean, and min / p25 / median / p75 / p90 / max.
// Null when no subnet saw a removal.
...
export function buildChainAxonRemovals(
  ...
  for (const row of list) {
    ...
    if (removers === 0) continue; // no removers: not a removal surface
  ...
  // Most active removing subnets first (by total AxonInfoRemoved events), tie-broken by netuid.
...
// Network-wide axon-removal activity, computed live: read the account_events AxonInfoRemoved
// stream over the window (observed_at >= now - windowDays, epoch ms), first as a single network
// aggregate (true distinct removers + newest observed_at) and then grouped by netuid for the
// per-subnet leaderboard, and shape with buildChainAxonRemovals. The newest-observed probe doubles
// as the cold-store guard: a null MAX(observed_at) skips the per-subnet read. The handler resolves
