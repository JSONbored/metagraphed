...
// COMPUTED LIVE per request from D1/KV/Postgres/RPC with no file anywhere. Every
...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Per-day stake & emission concentration trend (Gini, Nakamoto coefficient, top-10% share) over a 7d/30d/90d window for one subnet, served live from the Postgres-backed neuron_daily rollup at /api/v1/subnets/{netuid}/concentration/history; pass ?format=csv to download the per-day series as CSV (no static file).",
  ...
  artifact(
    ...
    "Validator-set & registration turnover (churn) for one subnet between a window's start and end snapshots — validators entered/exited + Jaccard retention, UID deregistrations, and a 0-100 stability score — served live from the Postgres-backed neuron_daily rollup at /api/v1/subnets/{netuid}/turnover (no static file).",
  ...
  artifact(
    ...
    "Cross-subnet momentum leaderboard: every subnet ranked by its change in stake, emission, validator, and neuron count between a window's start and end snapshots, with each subnet's share of network stake/emission and a network aggregate summary, computed live from the Postgres-backed neuron_daily rollup at /api/v1/subnets/movers (no static file).",
  ...
  artifact(
    ...
    "Network-wide validator/operator leaderboard: validator-permit identities grouped across all current subnet memberships and ranked by subnet footprint, UID footprint, validator trust, or cross-subnet stake/emission totals, computed live from the Postgres-backed neurons tier at /api/v1/validators (no static file).",
  ...
  artifact(
    ...
    "Balance-based top-holder leaderboard: every account (coldkey) with a nonzero free balance and/or delegated stake position, ranked by total TAO (free + delegated) or by cross-subnet stake flow (StakeAdded - StakeRemoved over a 7d/30d/90d window, #6886/#6887), served at /api/v1/accounts/top-holders (no static file). The coldkey/balance-centric counterpart to /api/v1/accounts, which is hotkey/neuron-centric. TWO TIERS (#9469): the net_flow_7d/30d/90d ranking is LIVE, recomputed daily from chain.account_events by the top-holders-flow projection lane, so captured_at advances on those sorts. free_tao/delegated_tao/total_tao are NOT live yet -- free_tao is already composed by that lane and starts ranking the moment account_balances has rows (#9501; the D1 sink landed in #9483, its producer is still infra-side), while delegated_tao needs a per-(hotkey, netuid) alpha pool total no current table holds (#9502) -- so those three sorts still answer from a FIXED materialization taken 2026-08-02 and their captured_at/last_updated do not advance. The holdings columns are null on a flow-ranked page, never zero.",
    "TopHoldersArtifact",
    COMPUTED_LIVE,
  ),
  artifact(
    ...
    "Cross-subnet detail for one validator identity: its validator_permit=1 rows aggregated across every subnet it operates in, computed live from the Postgres-backed neurons tier at /api/v1/validators/{hotkey} (no static file). The single-entity drill-in of /api/v1/validators.",
  ...
  artifact(
    ...
    "Cross-subnet staked-over-time + rewards-per-1000-TAO history for one validator: one point per snapshot_date, summed across every subnet it operates in that day, rolled up from the Postgres-backed neuron_daily tier at /api/v1/validators/{hotkey}/history (no static file).",
  ...
  artifact(
    ...
    "Per-UID metagraph (stake, trust, consensus, incentive, dividends, emission, validator_permit, rank, axon) for one subnet, served live from the Postgres-backed neurons tier at /api/v1/subnets/{netuid}/metagraph (no static file).",
  ...
  artifact(
    ...
    "A single neuron's metagraph state by UID, served live from the Postgres-backed neurons tier at /api/v1/subnets/{netuid}/neurons/{uid} (no static file).",
  ...
  artifact(
    ...
    "One subnet's consensus, economic, and governance hyperparameters (kappa, weight/activity settings, burn cost, liquid alpha, commit-reveal, yuma version, and more), refreshed daily and served live from the Postgres-backed subnet_hyperparams tier at /api/v1/subnets/{netuid}/hyperparameters (no static file).",
  ...
  artifact(
    ...
    "Append-only hyperparameter-change timeline for one subnet (subnet_hyperparams field snapshots on change), served live from the Postgres-backed subnet_hyperparams_history tier at /api/v1/subnets/{netuid}/hyperparameters/history; pass ?format=csv to download the page as CSV (no static file). Forward-only.",
  ...
  artifact(
    ...
    "Validators (validator_permit) of one subnet ranked by stake, served live from the Postgres-backed neurons tier at /api/v1/subnets/{netuid}/validators (no static file).",
  ...
  artifact(
    ...
    "Per-UID emission yield (emission/stake return rate) for one subnet over the current metagraph snapshot, ranked high to low with a distribution summary (subnet aggregate yield, mean, p25/median/p75/p90 percentiles), a validator/miner split, and a per-UID above/below-median label, served live from the Postgres-backed neurons tier at /api/v1/subnets/{netuid}/yield; pass ?format=csv to download the ranked neuron rows as CSV (no static file).",
    "SubnetYieldArtifact",
    COMPUTED_LIVE,
  ),
  artifact(
  ...
  artifact(
    ...
    "Per-UID daily metagraph history (stake/trust/emission/rank over time) for one UID, served live from the Postgres-backed neuron_daily rollup tier at /api/v1/subnets/{netuid}/neurons/{uid}/history (no static file).",
  ...
  artifact(
    ...
    "Per-subnet daily aggregate history (neuron/validator counts + stake/emission totals) for one subnet, served live from the Postgres-backed neuron_daily rollup tier at /api/v1/subnets/{netuid}/history (no static file).",
  ...
  artifact(
    ...
    "Append-only on-chain identity timeline for one subnet (SubnetIdentitiesV3 field snapshots on change), served live from the Postgres-backed subnet_identity_history tier at /api/v1/subnets/{netuid}/identity-history (no static file).",
  ...
  artifact(
    ...
    "The subnets where an account's hotkey is currently registered, served live from the Postgres-backed neurons tier at /api/v1/accounts/{ss58}/subnets (no static file).",
  ...
  artifact(
    ...
    "Personal chain identity for one account (epic #4301/5.4), the latest-only Postgres-backed account_identity row served live at /api/v1/accounts/{ss58}/identity (no static file). has_identity is false for the common case of an account that never called set_identity.",
  ...
  artifact(
    ...
    "Append-only diff-tracking timeline for one account's personal chain identity (epic #4301/5.2), served live from the Postgres-backed account_identity_history tier at /api/v1/accounts/{ss58}/identity-history (no static file).",
  ...
  artifact(
    ...
    "Network-wide idle-stake rollup: every subnet's own idle-stake scorecard (stake delegated to a currently-zero-dividends hotkey) ranked by idle_stake_alpha descending, plus the network total — the idle-delegation companion to chain-performance, computed live from the Postgres-backed neurons tier at /api/v1/chain/idle-stake (no static file).",
  ...
  artifact(
    ...
    "Network-wide validator-set turnover (churn) across all subnets between a window's start and end neuron_daily snapshots: each subnet's validators entered, exited, Jaccard retention, and a 0-100 stability score ranked into a leaderboard, a network rollup over the union of every subnet's validator hotkeys, and a distribution summary of the per-subnet stability scores (count, mean, min, p25, median, p75, p90, max), computed live from the Postgres-backed neuron_daily rollup at /api/v1/chain/turnover; pass ?format=csv to download the per-subnet leaderboard as CSV (no static file).",
...
export const API_ROUTES = [
  ...
  route(
    ...
    "/api/v1/accounts",
    "/metagraph/accounts.json",
    `Fetch the site-wide accounts leaderboard: every currently-registered hotkey (miners included, not just validator_permit=1 ones) grouped across all current subnet memberships, with cross-subnet stake/emission totals, stake dominance, a validator/miner UID breakdown, and top membership rows. Sort by total_stake (default), total_emission, subnet_count, uid_count, validator_count, stake_dominance, or last_active; limit caps the list (default ${ACCOUNTS_LIST_LIMIT_DEFAULT}, max ${ACCOUNTS_LIST_LIMIT_MAX}). Computed live from the Postgres-backed neurons tier. No 'Free'/spendable-balance or 'Total' column — no balance-tracking tier exists to source them from account_events/neurons.`,
    "short",
    ["accounts", "analytics"],
    csvRouteQuery([
      {
  ...
  route(
    ...
    "/metagraph/top-holders.json",
    `Fetch the balance-based top-holder leaderboard: every account (coldkey) with a nonzero free balance and/or delegated stake position, with free/delegated/total TAO columns matching the taostats-style Account/Free/Delegated/Total benchmark /api/v1/accounts explicitly cannot derive. Sort by total_tao (default), free_tao, delegated_tao, or cross-subnet stake flow over a window (net_flow_7d, net_flow_30d, net_flow_90d, #6886/#6887); limit caps the list (default ${TOP_HOLDERS_LIMIT_DEFAULT}, max ${TOP_HOLDERS_LIMIT_MAX}). free_tao is sourced from a direct System::Account chain-state scan (not event-reconstructed, so it can't drift); delegated_tao is this account's own stake positions across every hotkey/subnet, VALUED IN TAO: every position is non-root and therefore denominated in its subnet's alpha token, so each is multiplied by that netuid's alpha_price_tao from the latest daily subnet_snapshots row before summing (#8803) -- a DAILY cadence, so the price can lag up to ~24h behind the live economics tier, and a netuid with no usable price is excluded from the sum rather than counted as zero. total_tao adds it to free_tao, which is valid because both are TAO; net_flow_* is StakeAdded minus StakeRemoved over the window -- a negative value is a real net outflow, not a missing value -- and null-valued rows sort last on the net_flow_* keys. TWO TIERS, SELECTED BY SORT (#9469). The net_flow_* sorts are LIVE: the top-holders-flow projection lane recomputes all three windows daily from chain.account_events, so captured_at advances and the ranking is a real one. The free_tao/delegated_tao/total_tao sorts are NOT live and answer from a fixed materialization taken 2026-08-02, because account_balances has no rows yet -- its D1 sink landed in #9483 and the lane already composes the column, so free_tao starts ranking live the day the producer posts (#9501) -- and delegated_tao needs each (hotkey, netuid) alpha POOL total -- neurons.stake_tao only covers hotkeys holding a UID on that exact subnet, which is 22.8% of position rows, so recomputing from it would drop real top holders out of the ranking entirely. On those three sorts captured_at and last_updated do not advance: an account that has moved TAO since is misreported, and one first funded since is absent -- read that ranking as historical, and GET /api/v1/accounts/{ss58}/balance for the live per-account balance. On a net_flow_*-sorted page the three holdings columns are null rather than zero, because the live tier carries no balance source and a zero would assert an empty wallet.`,
    "short",
    ["accounts", "analytics"],
    csvRouteQuery([
      {
