...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Balance-based top-holder leaderboard: every account (coldkey) with a nonzero free balance and/or delegated stake position, ranked by total TAO (free + delegated) or by cross-subnet stake flow (StakeAdded - StakeRemoved over a 7d/30d/90d window, from the wallet_flow_daily rollup, #6886/#6887), served at /api/v1/accounts/top-holders (no static file). The coldkey/balance-centric counterpart to /api/v1/accounts, which is hotkey/neuron-centric. Recomputed every 6 hours (#9469) by a scheduled lane that reads account_balances (a direct System::Account chain-state scan, restored to a D1 sink in #9483), nominator_positions x neurons priced per subnet, and the account_events stake stream -- replacing the wallet_flow_daily rollup, which went with the indexer box. captured_at is the balance scan's own stamp and advances with each pass.",
    "TopHoldersArtifact",
    COMPUTED_LIVE,
  ),
  artifact(
