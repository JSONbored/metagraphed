...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Balance-based top-holder leaderboard: every account (coldkey) with a nonzero free balance and/or delegated stake position, ranked by total TAO (free + delegated) or by cross-subnet stake flow (StakeAdded - StakeRemoved over a 7d/30d/90d window, from the wallet_flow_daily rollup, #6886/#6887), served at /api/v1/accounts/top-holders (no static file). The coldkey/balance-centric counterpart to /api/v1/accounts, which is hotkey/neuron-centric. NOT LIVE (#9464): this route once computed from account_balances (a direct System::Account chain-state scan) joined with nominator_positions x neurons and wallet_flow_daily, but those tables were destroyed with the indexer box and no replacement writer exists, so it now answers from a FIXED materialization of that query taken 2026-08-02. captured_at/last_updated do not advance.",
    "TopHoldersArtifact",
    COMPUTED_LIVE,
  ),
  artifact(
