// Per-account stake-transfer footprint: which subnets one account (the origin
// coldkey) transferred stake out of over a recent window, broken down per subnet
// and rolled up into a transfer scorecard. Pure shaping (buildAccountStakeTransfers)
// + a thin D1 loader (loadAccountStakeTransfers); the Worker adds the REST envelope.
...
// /api/v1/subnets/{netuid}/stake-transfers. StakeTransferred moves stake between two
// coldkeys on the same subnet, so this measures an account's outbound transfer
// churn. The mover is the origin coldkey recorded on account_events.
