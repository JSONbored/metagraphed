...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Per-subnet coldkeys ownership leaderboard — who controls the subnet. Rolls the neurons up by controlling coldkeys (UID count, validator/miner split, total stake & emission, stake share) ranked by stake, plus an ownership-concentration scorecard over the coldkeys' stakes. The entity-level 'who owns it' drill-in of /concentration (which reports the metrics with coldkeys collapsed but never names the owners), served live from the neurons D1 tier at /api/v1/subnets/{netuid}/coldkeys (no static file).",
    "SubnetColdkeysArtifact",
  ),
  artifact(
    "subnet-performance",
...
export const API_ROUTES = [
  ...
  route(
    ...
    "/metagraph/subnets/{netuid}/coldkeys.json",
    "Fetch the per-subnet coldkeys ownership leaderboard — who controls the subnet: the neurons rolled up by controlling coldkeys (UID count, validator/miner split, total stake & emission, share of the subnet's stake) ranked by stake, plus an ownership-concentration scorecard (Gini/HHI/Nakamoto over the coldkeys' stakes). The entity-level 'who owns it' drill-in of GET /api/v1/subnets/{netuid}/concentration (which reports the metrics with coldkeys collapsed but never names the owners), computed live from the neurons D1 tier.",
    "short",
    ["subnets", "analytics"],
    [],
    [{ name: "netuid", schema: { type: "integer", minimum: 0 } }],
