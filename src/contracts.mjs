...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "One account's stake-transfer footprint per subnet over a recent window (7d/30d/90d): each subnet's StakeTransferred count with the first/last transfer timestamps, plus account totals, an HHI concentration of where its stake-transfer activity is focused, and the dominant subnet — summed live from the account_events D1 tier at /api/v1/accounts/{ss58}/stake-transfers (no static file). The account-level companion to /api/v1/chain/stake-transfers and /api/v1/subnets/{netuid}/stake-transfers, distinct from re-delegation churn in /api/v1/accounts/{ss58}/stake-moves.",
...
export const API_ROUTES = [
  ...
  route(
    ...
    "/metagraph/accounts/{ss58}/stake-transfers.json",
    "Fetch one account's stake-transfer footprint per subnet over a recent window (7d/30d/90d): each subnet's StakeTransferred count with the first and last transfer timestamps, plus account totals, an HHI concentration of where its stake-transfer activity is focused, and the dominant subnet — summed live from the account_events D1 tier. The account-level companion to GET /api/v1/chain/stake-transfers and GET /api/v1/subnets/{netuid}/stake-transfers, distinct from re-delegation churn in GET /api/v1/accounts/{ss58}/stake-moves.",
    "short",
    ["accounts", "analytics"],
    [
      {
