...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "Network-wide coldkeys ownership leaderboard — who controls Bittensor across every subnet. Rolls all neurons up by controlling coldkeys (subnet reach, UID count, validator/miner split, total stake & emission, network stake share) ranked by stake, plus a network ownership-concentration scorecard over the coldkeys' stakes. The named-entity drill-in of /chain/concentration (which reports the coldkeys-collapsed metrics but never names the owners) and the network-wide companion to /subnets/{netuid}/coldkeys, computed live from the neurons D1 tier at /api/v1/chain/coldkeys (no static file).",
