...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "One account's stake-transfer (between-coldkeys) footprint per subnet over a recent window (7d/30d/90d): each subnet's StakeTransferred count with the first/last transfer timestamps, plus account totals, an HHI concentration of where its outbound transfers are focused, and the dominant subnet — summed live from the account_events D1 tier at /api/v1/accounts/{ss58}/stake-transfers (no static file). Origin-side only (this account is the sender). The account-level companion to /api/v1/chain/stake-transfers, distinct from the within-account re-delegation churn of /api/v1/accounts/{ss58}/stake-moves.",
