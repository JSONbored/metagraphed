...
export const PUBLIC_ARTIFACTS = [
  ...
  artifact(
    ...
    "One account's stake-movement (re-delegation) footprint per subnet over a recent window (7d/30d/90d): each subnet's StakeMoved count with the first/last movement timestamps, plus account totals, an HHI concentration of where its movement activity is focused, and the dominant subnet — summed live from the account_events D1 tier at /api/v1/accounts/{ss58}/stake-moves (no static file). Keyed on the coldkey (the account initiating the move); the account-level companion to /api/v1/chain/stake-moves, distinct from /api/v1/accounts/{ss58}/stake-flow (StakeAdded/StakeRemoved net capital flow, hotkey-keyed).",
...
export const API_ROUTES = [
  ...
  route(
    ...
    "/metagraph/accounts/{ss58}/stake-moves.json",
    "Fetch one account's stake-movement (re-delegation) footprint per subnet over a recent window (7d/30d/90d): each subnet's StakeMoved count with the first and last movement timestamps, plus account totals, an HHI concentration of where its movement activity is focused, and the dominant subnet — summed live from the account_events D1 tier. Keyed on the coldkey (the account initiating the move); the account-level companion to GET /api/v1/chain/stake-moves, distinct from GET /api/v1/accounts/{ss58}/stake-flow (StakeAdded/StakeRemoved net capital flow, hotkey-keyed).",
    "short",
    ["accounts", "analytics"],
    [
      {
