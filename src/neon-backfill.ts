 ...
 *
 * `id` IS COPIED FROM D1 ON ALL FOUR, reversing this file's original call.
 * The id was thought to be "a tiebreaker that can never tie" because the
 * natural key is unique. It is load bearing twice over (#9954):
 *
 *   the WRITE path picks the latest revision by MAX(id), so two stores that
 *   number rows differently disagree about which revision is newest
 *
 *   observed_at is shared by every row in one sync, so two revisions written
 *   in the same millisecond are ordered by the id ALONE -- the natural key is
 *   unique in the production sample only because ticks are far apart, not
 *   because a collision cannot happen
 *
 * So D1 is the id authority until D1 is gone. Neon declares the column
 * GENERATED BY DEFAULT (migration 0004) and these lists carry it.
 */
