...
function normalizeChainAxonRemovalsSample(out) {
  ...
  // rollup uses the true distinct remover count (5, below the 6 per-subnet sum because a hotkey
  // removes an axon on both subnets), total 40 + 30 = 70 give 70/5 = 14, and the distribution
  // summarizes [10, 15]. The generic per-field generator cannot satisfy these events/removers
  // ratios itself.
  out.subnets = [
    {
      netuid: 1,
