...
// Cap the rows RETURNED by any single economics-trends read so the response and
// the builder's per-day rollup stay bounded. The `all` window has no date floor,
// so SQLite still reads the subnet_snapshots table ordered by snapshot_date — but
// at ~129 subnets × 365 days ≈ 47k rows that scan is small and finite, well under
// this cap. Exported so tests assert against the constant, not a literal.
