...
export function buildChainStakeTransferVolume(
  ...
  for (const row of list) {
    ...
    // Skip a null/blank/non-numeric volume, and defensively a negative aggregate: amount_tao is
    // always >= 0 for a transfer, so a negative SUM is malformed and would violate the schema's
    // volume_tao >= 0 (and total_volume_tao / distribution / avg minimums).
...
export async function loadChainStakeTransferVolume(
  ...
  // Defensive default so a direct caller that omits/garbles windowDays cannot produce a NaN cutoff
  // (the Worker handler always resolves it from analyticsWindow, 7d/30d).
  const days = Number.isFinite(windowDays)
    ? windowDays
    : CHAIN_STAKE_TRANSFER_VOLUME_WINDOWS[
        DEFAULT_CHAIN_STAKE_TRANSFER_VOLUME_WINDOW
      ];
