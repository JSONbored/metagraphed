...
export function buildCounterpartyRelationship(
  ...
  for (const row of list) {
    ...
    const observed = toIso(row.observed_at);
    if (observed != null) {
      firstObserved =
        firstObserved == null || observed < firstObserved
          ? observed
          : firstObserved;
      lastObserved =
    ...
    transfers.push({
      ...
      from,
      to,
      amount_tao: nullableNumber(row.amount_tao),
  ...
  return {
    ...
    first_seen_at: firstObserved,
    last_seen_at: lastObserved,
    limit: cap,
