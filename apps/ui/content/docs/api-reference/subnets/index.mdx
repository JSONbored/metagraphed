...
- [Subnet Price Share Composition](/docs/api-reference/subnets/subnet-price-share-composition) — `GET /api/v1/chain/subnet-price-share-composition`\
  Fetch a bounded daily artifact-normalized moving-price-share timeline for visual analysis: a fixed cohort of at most six netuids selected from the newest eligible recorded date, the same cohort across every returned day, and a six-decimal residual as `other`.
