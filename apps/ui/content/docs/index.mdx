...
<Callout type="info" title="Keyless stays keyless">
  ...
  per-domain — that never changes to a paid tier. A free, wallet-verified [API
  key](/settings) buys *more* headroom (currently 5× on the chain-events/deep-history routes,
  with more routes to follow) plus a usage dashboard — it never gates anything the anonymous
  tier already has.
</Callout>
