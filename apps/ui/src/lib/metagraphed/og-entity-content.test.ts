...
describe("entity preview content", () => {
  it("keeps the subnet identifier outside its three economic facts", () => {
    const card = subnetOgContent(19, {
      name: "Example subnet", alphaPriceTao: 0.08, emissionShare: 0.03, totalStakeAlpha: 1200,
    });
  ...
  it("preserves declared names and their hotkey rather than replacing identity with a count", () => {
    ...
    const card = validatorOgContent(hotkey, {
      name: "Declared name", logoPath: "/logos/example.png", logoHost: "example.com", subnetCount: 0,
    });
    expect(card).toMatchObject({ title: "Declared name", identifier: "Hotkey 5aaaaa…aaaaaz", logoPath: "/logos/example.png" });
  ...
  it("omits absent and malformed values while retaining observed zero", () => {
    for (const value of [null, undefined, NaN, Infinity, -1]) {
      expect(subnetOgContent(1, { alphaPriceTao: value, emissionShare: value, totalStakeAlpha: value }).stats).toEqual([]);
      expect(providerOgContent("example", { endpoints: value, surfaces: value, subnets: value }).stats).toEqual([]);
      expect(validatorOgContent("address", { subnetCount: value }).stats).toEqual([]);
    }
    expect(providerOgContent("example", { endpoints: 0, surfaces: 0, subnets: 0 }).stats).toHaveLength(3);
    expect(providerOgContent("example", { endpoints: 1.5 }).stats).toEqual([]);
    expect(subnetOgContent(1, { alphaPriceTao: 0, emissionShare: 0, totalStakeAlpha: 0 }).stats).toHaveLength(3);
  });

  it("retains curated logo paths and external-host fallback without current-health claims", () => {
    const data = { iconUrl: "https://metagraph.sh/logos/example.png", website: "https://example.com", name: "Example" };
  ...
  it("uses an already-resolved block number while retaining a supplied hash", () => {
    ...
    expect(blockOgContent(hash, 0)).toMatchObject({ title: "Block #0", identifier: "0xaaaa…aaaaaa" });
  ...
  it("uses the resolved call with its extrinsic reference, and degrades to identity only", () => {
    expect(extrinsicOgContent("123-0", "Balances.transfer")).toMatchObject({ title: "Balances.transfer", identifier: "Extrinsic 123-0" });
    for (const call of [null, undefined, "—", " "]) {
      expect(extrinsicOgContent("123-0", call)).toMatchObject({ title: "Extrinsic 123-0", identifier: null });
    }
  });

  it("preserves event zero and block zero as coordinates rather than statistics", () => {
    expect(eventOgContent(0, "0", "System.Success")).toMatchObject({ title: "System.Success", identifier: "Block #0 · Event #0" });
