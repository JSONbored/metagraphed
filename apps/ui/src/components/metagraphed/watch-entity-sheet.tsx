      ...
      {open ? (
        ...
        >
          <div
            ...
            aria-hidden
          />
          {/* eslint-disable-next-line no-restricted-syntax -- not a card shell: this is the bottom-sheet dialog surface itself (mobile-first rounded-t-xl/border-t geometry that upgrades to rounded/full border at sm, with its own max-h scroll region); <Panel> hardcodes `rounded border` and cannot express that responsive split without overriding every one of those classes */}
          <div className="relative z-[var(--mg-z-sticky)] mg-scroll max-h-[85vh] w-full overflow-y-auto rounded-t-xl border-t border-border bg-card p-4 sm:mx-4 sm:max-w-lg sm:rounded sm:border">
