    ...
    <Panel title={title} caption={caption} action={headerAction} className={className}>
      <div className="relative w-full" style={{ height }} aria-busy={loading || undefined}>
        {loading ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center text-13 text-ink-muted">
            {emptyLabel}
          </div>
        ) : (
          children
      ...
      {footer != null ? (
        <div
          className={classNames(
            "mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-border/70 pt-3 text-13 text-ink-muted",
          )}
        >
          {footer}
        </div>
