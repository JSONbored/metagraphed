...
// it can live as an "Inspect address" utility on /accounts (a whole page for
// one input box was the thing the redesign removed) -- the decoding logic,
// copy, and result cards are unchanged.
    ...
    <div className="space-y-4">
      ...
      {decoded?.extendedFormat ? (
        <ResultCard tone="warn" icon={AlertTriangle} title="Extended (2-byte) network prefix">
          <p>
            This address uses a prefix in the 64-127 range, which SS58 encodes across 2 bytes with a
            bit-packed scheme this tool doesn&apos;t decode — every network a Bittensor user would
            realistically encounter (Polkadot, Kusama, generic Substrate) uses a simple single-byte
            prefix instead, so this is either a wallet&apos;s byte encoding quirk or a much less
            common chain.
          </p>
        </ResultCard>
      ) : null}
