...
export const MEGA_PANELS: MegaPanel[] = [
  ...
  },
  {
    ...
    browse: [
      ...
      { to: "/chain/blocks", label: "Recent blocks", hint: "Newest first" },
      { to: "/chain/blocks", search: { limit: "100" }, label: "100 per page" },
      { to: "/chain/extrinsics", label: "Extrinsics", hint: "Transactions, newest first" },
      { to: "/accounts", label: "Accounts", hint: "Hotkey / coldkey lookup" },
      {
        to: "/chain/governance",
        label: "Governance",
        hint: "Sudo calls + AdminUtils config changes",
      },
      { to: "/chain/runtime", label: "Runtime", hint: "Spec-version upgrade history" },
    ],
    filters: [],
  },
  {
    ...
    browse: [
      { to: "/apis", label: "All surfaces" },
      { to: "/apis", search: { kind: "openapi" }, label: "OpenAPI" },
      { to: "/apis", search: { kind: "docs" }, label: "Docs" },
      { to: "/apis", search: { kind: "dashboard" }, label: "Dashboards" },
      { to: "/apis", search: { kind: "data" }, label: "Data artifacts" },
      { to: "/apis", search: { kind: "sse" }, label: "SSE streams" },
    ],
    filters: [
      { to: "/apis", search: { public_safe: "1" }, label: "Public-safe only" },
      { to: "/apis", search: { auth: "required" }, label: "Auth required" },
      { to: "/apis", search: { rate_limited: "1" }, label: "Rate-limited" },
    ],
  },
  {
    key: "endpoints",
    ...
    browse: [
      { to: "/apis/endpoints", label: "All endpoints" },
      { to: "/apis/endpoints", search: { category: "rpc" }, label: "Root RPC" },
      { to: "/apis/endpoints", search: { category: "wss" }, label: "WSS" },
      {
        to: "/apis/endpoints",
        search: { eligibility: "archive-capable" },
        label: "Archive-capable",
      },
      { to: "/apis/endpoints", search: { eligibility: "pool-member" }, label: "Pool-eligible" },
    ],
    filters: [
      { to: "/apis/endpoints", search: { health: "down" }, label: "Recent incidents" },
      { to: "/apis/endpoints", search: { health: "unknown" }, label: "Stale probes" },
    ],
  },
  {
    key: "providers",
