...
| The router reused one entity-detail skeleton for every slow destination.                                                                                  | Landing, directory, analytic, comparison, settings, and reading routes could briefly reshape into an unrelated fact ledger and records table, making navigation feel slower and less trustworthy.                               | The pending fallback now selects from the target pathname and renders an archetype-specific first-paint shape for landing, directory/stream, entity, block, operational, comparison, settings/product-tool, and reading routes. Decorative placeholders invent no values, one polite busy status owns the announcement, and every archetype is checked at phone, tablet, and desktop widths in both themes.                       |
| Deferred evidence regions repeated viewport-scheduling copy in both their empty panel and footnote.                                                       | Public pages read like an internal performance dashboard, and the prose disappeared into a different chart or table shape as the request began.                                                                                 | Before activation, each region now renders the same data-shaped chart, table, rail, or fact skeleton used while its query is pending. Stable domain and methodology notes replace implementation copy; request deferral, deep links, scoped errors, and retries are unchanged, including the raw ownership disclosure.                                                                                                            |
...
| Hub explanations placed paragraph-length answers inside 28px analytic-question headings.                                                                  | The landing, subnet, validator, API, provider, endpoint, schema, and chain hubs ended in oversized walls of headline text; phones spent several screens on secondary context, and the full answer polluted the heading outline. | Each explanation now keeps a concise real `h2` separate from its 16px measured paragraph, arranged as a ruled two-column reading ledger from tablet upward and a compact stack on phones. All copy remains server-rendered. Across the eight deterministic 375px fixtures, matched page height fell by 390–864px (median 583px, or 11.4%) with no horizontal overflow; this is a local layout measurement, not a load-time claim. |
