...
let readHealthMetaKv = () => {
  throw new Error("provider report used before configureProviderReport()");
};
let readEconomicsCurrentKv = () => {
  throw new Error("provider report used before configureProviderReport()");
};

export function configureProviderReport(deps) {
...
export async function handleProviderReport(request, env, slug, url) {
  ...
  const [subnetMeta, healthRows, surfaceKindRows, economicsRows, meta] =
    await Promise.all([
      ...
      needSurfaceStatus && includeSurfaces
        ...
        : null,
      includeEconomics ? resolveEconomicsRows(env) : null,
