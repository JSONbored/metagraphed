...
// 7d/30d window, read from the account_events AxonInfoRemoved stream. The removal-side companion to
// chain-serving; mirrors chain-registrations: window + limit + ?format=csv params, HEAD probes
// normalized through the GET cache key so they cannot bypass the edge cache and repeatedly force the
// network-wide aggregations, cache keyed on the analytics cron freshness. The leaderboard is fixed
// to most-active-first (total AxonInfoRemoved events).
