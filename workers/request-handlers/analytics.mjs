...
// GET /api/v1/chain/axon-removals: network-wide axon-removal (teardown) activity across every
// subnet over a 7d/30d window, read from the account_events AxonInfoRemoved stream. The removal-side
// companion to chain/serving (announcement activity); mirrors chain-stake-moves: window + limit
// params, HEAD probes normalized through the GET cache key so they cannot bypass the edge cache and
// repeatedly force the network-wide aggregations. The leaderboard is fixed to most-active-first
// (total AxonInfoRemoved events).
