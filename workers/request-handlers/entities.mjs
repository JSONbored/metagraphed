...
// GET /api/v1/accounts/{ss58}/deregistrations: the account's per-subnet NeuronDeregistered
// footprint over a 7d/30d/90d window — deregistration count + first/last timestamps per subnet, an
// HHI concentration of where its deregistration activity is focused, and the dominant subnet.
// account_events-derived (source "chain-events"). Cold/absent store → schema-stable zeros (never 404).
