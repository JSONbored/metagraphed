...
// GET /api/v1/accounts/{ss58}/stake-transfers: the account's per-subnet StakeTransferred footprint
// over a 7d/30d/90d window — transfer count + first/last timestamps per subnet, an HHI
// concentration of where its between-account transfer activity is focused, and the dominant subnet.
// account_events-derived (source "chain-events"). Cold/absent store → schema-stable zeros.
