...
describe("rowsToCsv", () => {
  ...
  test("mitigates spreadsheet formula injection by prepending tab to =, +, -, @", () => {
    const csv = rowsToCsv([
      { cell: "=SUM(1,2)" },
      { cell: "+100" },
      { cell: "-50" },
    ...
    const lines = csv.split("\r\n");
    assert.equal(lines[1], '"\t=SUM(1,2)"');
    assert.equal(lines[2], "\t+100");
    assert.equal(lines[3], "\t-50");
    assert.equal(lines[4], "\t@username");
  });
});
...
describe("contentDispositionFilename", () => {
  test("strips path separators (slash and backslash)", () => {
    assert.equal(
      contentDispositionFilename("neurons/42\\export"),
      "neurons42export",
    );
  });

  test("strips ASCII control characters (0x00-0x1f and 0x7f)", () => {
    assert.equal(
      contentDispositionFilename("export\r\n\x00\x1f\x7fdata"),
      "exportdata",
    );
  });

  test("escapes double quotes", () => {
    assert.equal(contentDispositionFilename('neurons"42"'), 'neurons\\"42\\"');
  });

  test("uses fallback 'export' if input is nullish or empty", () => {
    assert.equal(contentDispositionFilename(null), "export");
    assert.equal(contentDispositionFilename(undefined), "export");
    assert.equal(contentDispositionFilename(""), "export");
  });
});
...
describe("csvResponse", () => {
  ...
  test("Content-Disposition attachment header uses the sanitized filename with .csv extension", () => {
