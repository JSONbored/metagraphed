    ...
    try {
      ...
      const dispatchReadRoutes = async (sql: postgres.TransactionSql) => {
        ...
        if (chainFees) {
          ...
          // signer IS NOT NULL excludes unsigned inherents (#8809); no COALESCE
          // on the ORDER BY expressions either, since PERCENTILE_CONT already
          // ignores NULLs -- a signed extrinsic with an unrecorded fee should
          // drop out of the median, not count as a free (0) transaction.
