...
export async function handleRequest(request, env = {}, ctx = {}) {
  ...
  if (url.pathname === "/api/v1" || url.pathname.startsWith("/api/v1/")) {
    ...
    // GET /api/v1/chain/consensus: network-wide neurons aggregate — edge-cache busts
    // on the newest neuron captured_at across ALL subnets, not the health prober
    // tick (like the per-subnet concentration route, but network-scoped).
    if (resolved.url.pathname === "/api/v1/chain/consensus") {
      return withEdgeCache(
        request,
        ctx,
        env,
        "chain-consensus",
        () => handleChainConsensus(request, env, resolved.url),
        null,
        (edgeEnv) => readNeuronsCacheStamp(edgeEnv),
      );
    }
...
function isMainnetOnlyApiPath(pathname) {
  return (
    ...
    pathname === "/api/v1/chain/consensus" ||
