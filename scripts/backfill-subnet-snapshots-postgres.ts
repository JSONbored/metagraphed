...
//
// Usage:
//   node scripts/backfill-subnet-snapshots-postgres.ts [--out <path>]
...
export function fetchPage(
  ...
  const rows = (parsed?.[0] as Row | undefined)?.results;
...
export function sqlLiteral(column: string, value: unknown): string {
...
export function rowTuple(row: Row): string {
...
export function insertStatement(rows: Row[]): string {
...
export function buildBackfillSql(
  ...
  const statements = (chunkRows(allRows, rowsPerStatement) as Row[][]).map(
    insertStatement,
  );
  return (
    `-- Generated by scripts/backfill-subnet-snapshots-postgres.ts -- ` +
...
export async function fetchAllRows(
  ...
  for (;;) {
    ...
    allRows.push(...page);
    afterRowid = page[page.length - 1].rowid as number;
    console.log(
...
async function main(): Promise<void> {
  ...
  const statementCount = (chunkRows(allRows, args.rowsPerStatement) as Row[][])
    .length;
  console.log(
