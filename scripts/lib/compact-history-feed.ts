...
export async function compactHistoryFeed<
  F extends HistoryAccountFeed | HistoryExtrinsicFeed,
  V extends number | string | boolean | null,
>(
  codec: {
    name: string;
    validate(input: unknown, selected: F["selection"]): F;
    base(feed: F): string;
    encoding: F["encoding"];
    decode(raw: Uint8Array): { token: string; values: V[] }[] | undefined;
    encode(
      entries: readonly { token: string; values: V[] }[],
    ): Uint8Array | undefined;
    digest(entries: readonly { token: string; values: V[] }[]): string;
  },
  input: unknown,
  ...
  const config = OptionsSchema.parse(options),
    feed = codec.validate(input, selection);
  const base = codec.base(feed);
  ...
  function register(object: HistoryObject) {
    ...
    if (prior && (prior.etag !== object.etag || prior.bytes !== object.bytes))
      throw new Error(`${codec.name} compaction object identity conflict`);
  ...
  async function read(
    ...
    if (
      ...
      throw new Error(`${codec.name} compaction source content hash mismatch`);
    return raw;
  }
  async function write(
    ...
    if (stats.writtenBytes + raw.length > config.maxWriteBytes)
      throw new Error(`${codec.name} compaction write budget exceeded`);
    ...
    if (object.key !== key || object.bytes !== raw.length)
      throw new Error(`${codec.name} compaction output identity mismatch`);
    outputs.push(object);
    ...
    if (digest(restored) !== digest(raw))
      throw new Error(`${codec.name} compaction readback mismatch`);
    return object;
  }
  ...
  async function children(node: HistoryFeedNode): Promise<HistoryFeedNode[]> {
    ...
    if (
      ...
      throw new Error(`${codec.name} compaction directory census mismatch`);
  ...
  async function collect(node: HistoryFeedNode): Promise<Tree> {
    ...
    if (++stats.pages > config.maxPages)
      throw new Error(`${codec.name} compaction page budget exceeded`);
    if (previous !== undefined && previous >= node.first)
      throw new Error(`${codec.name} compaction page order mismatch`);
    previous = node.last;
  ...
  function transcode(tree: Tree, original: Uint8Array) {
    ...
    if (raw.length !== node.decodedBytes)
      throw new Error(`${codec.name} compaction decoded size mismatch`);
    stats.decodedPageBytes += raw.length;
    let entries =
      feed.encoding === codec.encoding ? codec.decode(raw) : undefined;
    if (!entries) {
      const lines = text.decode(raw);
      if (!lines.endsWith("\n"))
        throw new Error(
          `Truncated ${codec.name.toLowerCase()} compaction page`,
        );
      entries = lines
        .slice(0, -1)
        .split("\n")
        .map((line) => {
          if (line[166] !== "\t")
            throw new Error(
              `Invalid ${codec.name.toLowerCase()} compaction record`,
            );
          return {
    ...
    const encoded = codec.encode(entries);
    if (
      ...
      throw new Error(`${codec.name} compaction page census mismatch`);
    ...
    if (pageDigest !== digestPage(recovered))
      throw new Error(`${codec.name} compaction round-trip content mismatch`);
    ...
    if (stats.newPageBytes > config.maxWriteBytes)
      throw new Error(`${codec.name} compaction staging budget exceeded`);
    ...
    for (const group of groups.values()) {
      ...
      for (let i = 1; i < group.length; i++) {
        ...
        if (prior.offset + prior.length > (group[i].node as Leaf).offset)
          throw new Error(`${codec.name} compaction overlapping source pages`);
      }
  ...
  if (!feed.root) {
    if (config.path.length)
      throw new Error(`${codec.name} compaction path outside tree`);
    return {
      manifest: feed,
      stats,
      budget,
  ...
  if (stats.entries !== target.rows)
    throw new Error(`${codec.name} compaction round-trip census mismatch`);
  ...
  const manifest = codec.validate(
    {
      ...feed,
      encoding: tree.changed ? codec.encoding : feed.encoding,
      root,
    },
    selection,
  );
