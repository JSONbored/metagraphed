...
- `/metagraph/subnets/{netuid}/transfer-volume.json`: schema for per-subnet native-TAO transfer analytics (total Balances.Transfer volume + count among accounts currently registered on the subnet, distinct senders/receivers, top senders/receivers ranked by volume, and top-sender concentration) over a 7d/30d/90d window, served live from the account_events D1 stream at `GET /api/v1/subnets/{netuid}/transfer-volume` (no static file). Transfer rows carry no netuid field — attribution joins the current neurons snapshot.
...
- `/api/v1/subnets/{netuid}/transfer-volume`: fetch per-subnet native-TAO transfer analytics (total volume + count, distinct senders/receivers, top senders/receivers leaderboards, top-sender concentration) for one subnet over a `?window=7d|30d|90d` with `?limit=` capping each leaderboard (default 20, max 100; live from the account_events D1 stream).
