...
- `/api/v1/chain/event-summary`: fetch a network-wide windowed `account_events` summary across every subnet — counts by event kind and coarse category, distinct hotkey/coldkey counts, per-kind subnet reach, TAO/alpha sums where applicable, first/last evidence bounds, plus a newest-first evidence slice; `?window=7d|30d|90d` (default 30d), `?limit` <=50 (computed live from the `account_events` D1 tier; schema-stable empty block when cold). Companion to `GET /api/v1/subnets/{netuid}/event-summary`.
