...
- `/metagraph/surfaces/readiness.json`: schema for the per-surface integration-readiness scorecard — each curated public surface's auth + rate-limit + schema clarity composed into a 0-100 `readiness_score` and a tier — served live at `GET /api/v1/surfaces/readiness` (no static file).
...
- `/api/v1/surfaces/readiness`: per-surface integration-readiness scorecard. Each curated public surface's auth detail + rate-limit structure + schema availability is composed into clarity sub-scores, a 0-100 `readiness_score`, and a `readiness_tier` (`ready`/`callable-unverified`/`blocked`/`reference`). Filter by `netuid`, `kind`, `provider`, `authority`, `readiness_tier`, `callable`; search/sort/paginate. Display-only; live up/down is at `/api/v1/subnets/{netuid}/health`.
