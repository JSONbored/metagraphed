...
- `/metagraph/chain/yield.json`: schema for the network-wide emission yield (return on stake) aggregated across every subnet's neurons — stake/emission totals, stake-weighted network/validator/miner aggregate yields, a per-neuron yield distribution for each of those three lenses, and a top-yielders leaderboard — computed live from the `neurons` D1 tier at `GET /api/v1/chain/yield` (no static file).
...
- `/api/v1/chain/yield`: fetch network-wide emission yield (return on stake) aggregated across every subnet's neurons — stake/emission totals, stake-weighted network/validator/miner aggregate yields, a per-neuron yield distribution (count, mean, min/p25/median/p75/p90/max) for each of those three lenses, and a `?limit`-capped top-yielders leaderboard (computed live from the `neurons` D1 tier; schema-stable nulls when cold).
