...
- `/metagraph/chain/consensus.json`: schema for the network-wide Yuma-consensus performance distribution (per-signal trust/consensus/incentive/dividends count, mean, and min/p25/median/p75/p90/max spread, plus active/validator/miner counts) aggregated across every subnet's neurons, computed live from the `neurons` D1 tier at `GET /api/v1/chain/consensus` (no static file).
...
- `/api/v1/chain/consensus`: fetch the network-wide Yuma-consensus performance distribution aggregated across every subnet's neurons, with a count, mean, and min/p25/median/p75/p90/max spread for each signal (trust, consensus, incentive, dividends) plus active/validator/miner counts (computed live from the `neurons` D1 tier; schema-stable nulls when cold).
