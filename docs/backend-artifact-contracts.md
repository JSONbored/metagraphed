...
- `/metagraph/providers/{slug}/report.json`: schema for the live cross-subnet provider performance report — identity, per-subnet surface-kind breakdown, live probe health, and economics — served live at `GET /api/v1/providers/{slug}/report` (no static file).
...
- `/api/v1/providers/{slug}/report`: fetch a live cross-subnet performance report for one provider. `dimensions` (optional) selects a subset of `identity,surfaces,health,economics` (default all). Composes provider identity, per-subnet surface-kind cells, live probe health rollups (`WHERE provider = ?` on D1 `surface_status`), and economics rows for every subnet the provider serves. Mainnet-only; 404 when the provider artifact is missing.
