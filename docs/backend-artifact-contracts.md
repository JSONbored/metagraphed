...
- `/metagraph/chain/onboarding.json`: schema for network-wide neuron registration inflow over a 7d/30d window across the subnets with observed registration activity — a per-subnet leaderboard (registration count, distinct hotkeys, registrations per hotkey), a network rollup with the true distinct hotkey count, and a distribution of per-subnet re-registration intensity — computed live from the `account_events` NeuronRegistered stream at `GET /api/v1/chain/onboarding` (no static file).
...
- `/api/v1/chain/onboarding`: fetch network-wide neuron registration inflow over a `?window=7d|30d` across the subnets with observed registration activity — a per-subnet leaderboard (registration event count, distinct registered hotkeys, registrations per hotkey) ranked by total registrations (`?limit` <=100), a network rollup with the true distinct hotkey count, and a distribution of per-subnet re-registration intensity (computed live from the `account_events` NeuronRegistered stream; schema-stable empty block when cold).
