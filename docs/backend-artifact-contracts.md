...
- `/metagraph/registry/coverage-matrix.json`: schema for the surface kind × subnet coverage matrix — which surface kinds each subnet has and their live health, one row per subnet ordered by completeness_score descending — served live at `GET /api/v1/registry/coverage-matrix` (no static file).
...
- `/api/v1/registry/coverage-matrix`: fetch the surface kind × subnet coverage matrix composed live from the registry surfaces list and the D1 health tier. `kinds` (optional) selects which surface kinds appear as columns (default: `openapi,subnet-api,sse,data-artifact,sdk`); `health` (optional, default `true`) includes live `ok_count` and `avg_latency_ms` per cell. Rows ordered by `completeness_score` descending; null cells indicate no surface of that kind registered for the subnet.
