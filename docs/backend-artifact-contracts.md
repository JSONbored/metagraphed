...
- `/metagraph/chain/axon-removals.json`: schema for network-wide axon-removal (teardown) activity over a 7d/30d window across the subnets with observed removal activity — a per-subnet leaderboard (AxonInfoRemoved count, distinct removers, removals per remover), a network rollup with the true distinct remover count, and a distribution of per-subnet re-teardown intensity — the removal-side companion to `chain/serving` (announcement activity), computed live from the `account_events` AxonInfoRemoved stream at `GET /api/v1/chain/axon-removals` (no static file).
...
- `/api/v1/chain/axon-removals`: fetch network-wide axon-removal (teardown) activity over a `?window=7d|30d` across the subnets with observed removal activity — a per-subnet leaderboard (AxonInfoRemoved event count, distinct removers, removals per remover) ranked by total removals (`?limit` <=100), a network rollup with the true distinct remover count, and a distribution of per-subnet re-teardown intensity — the removal-side companion to `chain/serving` (computed live from the `account_events` AxonInfoRemoved stream; schema-stable empty block when cold).
