...
- `/metagraph/accounts/{ss58}/deregistrations.json`: schema for an account's neuron-deregistration footprint per subnet over a 7d/30d/90d window (per-subnet NeuronDeregistered count with first/last deregistration timestamps, account totals, an HHI concentration, and the dominant subnet) — the account-level companion to `chain/deregistrations`, served live from the `account_events` D1 tier at `GET /api/v1/accounts/{ss58}/deregistrations` (no static file).
...
- `/api/v1/accounts/{ss58}/deregistrations`: fetch an account's neuron-deregistration footprint per subnet over a `?window=7d|30d|90d` — per-subnet NeuronDeregistered count with the first/last deregistration timestamps, account totals, an HHI concentration of where its deregistration activity is focused, and the dominant subnet — the account-level companion to `chain/deregistrations` (live from the `account_events` D1 tier).
