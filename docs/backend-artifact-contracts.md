...
- `/metagraph/subnets/{netuid}/turnover/changes.json`: schema for validator-set turnover change detail (validator hotkeys entered/exited + UID reassignments) between a window's start and end snapshots, served live from the neuron_daily D1 rollup at `GET /api/v1/subnets/{netuid}/turnover/changes` (no static file).
...
- `/metagraph/accounts/{ss58}/counterparties/{counterparty}.json`: schema for pair-level account/counterparty fund-flow detail (bounded summary totals + recent native-TAO transfer evidence), served live from the `account_events` D1 tier at `GET /api/v1/accounts/{ss58}/counterparties/{counterparty}` (no static file).
