...
- `/metagraph/chain/stake-flow.json`: schema for network-wide net stake flow over a 7d/30d/90d window (total TAO staked vs unstaked, net flow, stake/unstake event counts), summed live from the `account_events` stream at `GET /api/v1/chain/stake-flow` (no static file).
...
- `/api/v1/chain/stake-flow`: fetch network-wide net stake flow over a `?window=7d|30d|90d` — total TAO staked (StakeAdded) vs unstaked (StakeRemoved), net capital flow, and stake/unstake event counts; `?direction=all|in|out` narrows to one side (live from the `account_events` stream; schema-stable zeros when cold).
