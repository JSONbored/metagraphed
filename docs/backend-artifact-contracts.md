...
- `/metagraph/subnets/{netuid}/coldkeys.json`: schema for a subnet's coldkeys ownership leaderboard (the neurons rolled up by controlling coldkeys — UID count, validator/miner split, total stake & emission, stake share — ranked by stake, plus an ownership-concentration scorecard), the entity-level "who owns it" drill-in of `/concentration`, served live from the neurons D1 tier at `GET /api/v1/subnets/{netuid}/coldkeys` (no static file).
...
- `/api/v1/subnets/{netuid}/coldkeys`: fetch the per-subnet coldkeys ownership leaderboard — the neurons rolled up by controlling coldkeys (UID count, validator/miner split, total stake & emission, stake share) ranked by stake, plus an ownership-concentration scorecard — the entity-level "who owns it" drill-in of `/concentration` (live from the neurons D1 tier).
