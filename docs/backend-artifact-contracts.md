...
- `/metagraph/accounts/{ss58}/event-summary.json`: schema for an account's windowed event summary (counts by event kind/category, distinct hotkey/coldkey counts, TAO/alpha sums, subnet footprint, first/last evidence bounds, and recent evidence), served live from the `account_events` D1 tier at `GET /api/v1/accounts/{ss58}/event-summary` (no static file).
...
- `/api/v1/accounts/{ss58}/event-summary`: fetch a windowed event summary for one account (hotkey or coldkey) — counts by event kind/category, distinct hotkey/coldkey counts, TAO/alpha sums, subnet footprint, first/last evidence bounds, plus recent evidence; `?window=7d|30d|90d`, `?limit` <=50, optional `?netuid=` (live from the `account_events` D1 tier).
